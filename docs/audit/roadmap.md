...
### 7.40. `Topbar.tsx` писал в несуществующее поле `._id` на таймере, а `logger.ts` гасил детали любой ошибки в `meta: {}` (F45, ⚠️ реализовано, тесты зелёные, смерджено в `main` PR [#114](https://github.com/danny-swan/taskflow-app/pull/114), ждёт ручную проверку)
...
**Статус:** ⚠️ реализовано, ждёт ручной проверки: убедиться, что таймер в Topbar по-прежнему чистится при анмаунте (нет утечек таймеров), и что новые ошибки в `taskflow.log` несут реальный `message`/`stack` вместо `meta: {}`.

---

### 7.41. `pinWorkspacePointers`/`reconcileLocalPlaceholder` — один необёрнутый блок и три необёрнутых шага — падение в одном шаге гасило, прошёл ли остальные (F46, ⚠️ реализовано, тесты зелёные, смерджено в `main` PR [#114](https://github.com/danny-swan/taskflow-app/pull/114), ждёт ручную проверку)
...
**Статус:** ⚠️ реализовано, ждёт ручной проверки: при возникновении подобной ошибки в будущем в `taskflow.log` должна появиться чёткая пошаговая запись — какой из 4 шагов сломался, а не общий opaque failure.

---

### 7.42. `sync_devices` upsert упирался в RLS при втором аккаунте на том же устройстве — `client_id` никогда не сбрасывался при смене аккаунта (F47, Bug 3, ⚠️ реализовано, тесты зелёные, смерджено в `main` PR [#114](https://github.com/danny-swan/taskflow-app/pull/114), ждёт ручную проверку)
...
**Статус:** ⚠️ реализовано, ждёт ручной проверки: на устройстве, где ранее был аккаунт A, войти под аккаунтом B — в `taskflow.log` больше не должно быть `[sync/device] upsert failed`, синх для B должен идти без ошибок.

---

### 7.43. `pullAll()` не ревалидировал сессию после старта — выход/смена аккаунта в середине пулла давали серию `permission denied` по всем рабочим пространствам (F48, ⚠️ реализовано, тесты зелёные, смерджено в `main` PR [#114](https://github.com/danny-swan/taskflow-app/pull/114), ждёт ручную проверку)
...
**Статус:** ⚠️ реализовано, ждёт ручной проверки — это cosmetic/log-noise фикс, прямой пользовательский шаг проверить тяжело (нужно воссоздать ту же гонку в логах и сравнить количество `permission denied` до/после) — принимается как закрытый по ~745/745 тестам + отсутствию новых ошибок в обычная работе.
