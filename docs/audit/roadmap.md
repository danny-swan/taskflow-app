...
**24 миграции** (`supabase/migrations/0001`→`0024`) — от базовой схемы до полной sync-схемы, entitlements/payments, GRANT-хардненинга (3 итерации), payment_methods + pg_cron автопродление, security-хардненинг Wave 2/3/4 (0020–0024, вкл. `rate_limits`). Полная таблица — Приложение А.
...
## 8. Финальный чек-лист: что осталось перед merge `develop` → `main`

Обновление от 11.07.2026. **Wave 1–4 полностью на проде** (F1-F5, N4-N18). Бэкенд-часть (миграции `0001`–`0024`, edge-функции) закрыта. Ниже — всё, что ещё стоит между текущим `develop` и первым не-pre-release релизом v1.0.0. Раздел дополняет открытые пункты раздела 6 конкретикой и статусами.
...
5. **Провести один реальный тестовый платёж** на минимальный тариф с реальной карты (можно свою) и убедиться:
   ...
   - фискальный чек ушёл в ФНС (ЛК ЮKassa → вкладка «Чеки»).
