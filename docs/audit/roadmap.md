...
### v1.1.0 — пространства (личные + общие) (03.08.2026)

> Минорный релиз, объединяющий всю волну `feat/workspaces` (65 коммитов от `v1.0.3`, PR #74–#106) и последующие пост-мерж фиксы F36–F39 (PR #107–#109). Содержание: несколько личных пространств, общие пространства с ролями и приглашениями по `TF-XXXXXX`, presence, вкладка «История», тарифные лимиты пространств, per-account локальные слоты для free-тарифа, волна фиксов надёжности и навигации. Серверная часть — миграции `0027`–`0040`, все ✅ применены на прод (детали — Приложение А). Ветка `feat/workspaces` смёржена в `main` 02.08.2026 (см. §7.29–§7.30 и ADR 0027).

| Событие | Коммит / ссылка | Комментарий |
|---|---|---|
| Мерж `feat/workspaces` → `main` | PR [#106](https://github.com/danny-swan/taskflow-app/pull/106) | Волна A/B/C + фиксы F7–F35, ADR 0005–0027. Ручной контракт применения миграций зафиксирован в ADR 0027 |
| PR #107 — `fix(free-bootstrap)` | [PR #107](https://github.com/danny-swan/taskflow-app/pull/107), коммит `00edae1` | F36 (§7.31): приветственная задача не отображалась на свежем free-аккаунте — сид справочников без явного `workspace_id`. ADR 0028 |
| PR #108 — `fix(free-logout)` | [PR #108](https://github.com/danny-swan/taskflow-app/pull/108), коммит `079985b` | F37 (§7.32): задача, созданная перед быстрым выходом, могла пропасть — очередь нативных записей + сохранение free-слота при выходе. ADR 0029 |
| PR #109 — `fix(navigation)` | [PR #109](https://github.com/danny-swan/taskflow-app/pull/109), коммит `2279e95`, merge `56964a5` | F38/F39 (§7.33, §7.34): дефолтная вкладка при входе/смене пространства + секции настроек по хэшу маршрута (HashRouter). ADR 0030 |
| Релиз десктопа v1.1.0 | [тег v1.1.0](https://github.com/danny-swan/taskflow-app/releases/tag/v1.1.0), коммит `56964a5`, [CI run 30825879602](https://github.com/danny-swan/taskflow-app/actions/runs/30825879602) | Собраны Windows (NSIS + MSI RU/EN + portable) и macOS (universal dmg), `latest.json` (`version: 1.1.0`) — юзеры v1.0.0–1.0.3 получат обновление через auto-updater. 685 vitest, E2E Playwright, Deno-тесты Edge Functions зелёные. Версия синхронизирована из тега CI-ом (в git-истории `package.json`/`tauri.conf.json`/`Cargo.toml` остаются с прежними значениями — как и в v1.0.2/v1.0.3) |

**Применение на прод:** серверные миграции `0027`–`0040` применены до релиза (по ходу волны, см. Приложение А). Клиентские фиксы F36–F39 уехали на прод-юзеров с v1.1.0. Открытая находка F35 (§7.30) требует миграцию `0041_revoke_execute_log_task_activity.sql` — в v1.1.0 НЕ входит, остаётся в очереди.

### v1.1.1 — фиксы после ручной проверки волны пространств (03.08.2026)

> Патч-релиз поверх v1.1.0: пять клиентских находок, найденных при ручной проверке пользователем, — сброс дефолтной вкладки и секции настроек по хэшу были ещё в v1.1.0, здесь закрыты F40–F44. Серверная часть — миграции `0042`/`0043` (аватар-цвет и RPC публичных профилей участников), применены на прод 03.08.2026 до релиза. Новых миграций в F42/F44 нет.

| Событие | Коммит / ссылка | Комментарий |
|---|---|---|
| PR #110 — `feat(profile)` | [PR #110](https://github.com/danny-swan/taskflow-app/pull/110) | F40/F41 (§7.35, §7.36): реальные ники и аватары участников через SECURITY DEFINER RPC `get_workspace_member_profiles` (миграция `0043`) и явный `profiles.avatar_color` (миграция `0042`) + редизайн «Настройки → Аккаунт». ADR 0031, 0032 |
| PR #111 — `fix(sync)` | [PR #111](https://github.com/danny-swan/taskflow-app/pull/111), squash `531b176` | F42/F43 (§7.37, §7.38): «Покинуть пространство» не доезжало до сервера — `op='delete'` уходил PostgREST-upsert'ом и падал на INSERT-политике RLS с 42501; теперь гашение идёт `UPDATE`'ом узким патчем. Плюс тексты: «Канбан» → «Карточки», раздел FAQ о пространствах, шаг «Пространства» в туре. ADR 0033 |
| PR #112 — `fix(workspaces)` | [PR #112](https://github.com/danny-swan/taskflow-app/pull/112), squash `160c9e4` | F44 (§7.39): наблюдатель не мог выйти из пространства — ссылка на `/workspace-settings` была за гейтом `canEdit`, теперь видна всем участникам (экран и так read-only для не-владельца). Плюс правки текстов тура и FAQ по итогам ручной проверки. ADR 0034 |
| Релиз десктопа v1.1.1 | [тег v1.1.1](https://github.com/danny-swan/taskflow-app/releases/tag/v1.1.1), коммит `160c9e4`, [CI run 30850620390](https://github.com/danny-swan/taskflow-app/actions/runs/30850620390) | Собраны Windows (NSIS + MSI RU/EN + portable) и macOS (universal dmg), `latest.json` (`version: 1.1.1`) — юзеры v1.0.0–1.1.0 получат обновление через auto-updater. 89 файлов / 742 vitest, E2E Playwright, Deno-тесты Edge Functions зелёные. Версия синхронизирована из тега CI-ом (в git-истории `package.json`/`tauri.conf.json`/`Cargo.toml` остаются с прежними значениями — как в v1.0.2/v1.0.3/v1.1.0) |

**Применение на прод:** миграции `0042`/`0043` применены 03.08.2026 (Приложение А); F42/F43/F44 — чисто клиентские, без миграций и без изменений RLS. Открытая находка F35 (§7.30) с миграцией `0041_revoke_execute_log_task_activity.sql` в v1.1.1 НЕ входит, остаётся в очереди.

### v1.1.2 — видимость ошибок в синхе + самоисцеление устройства + REVOKE EXECUTE на проде (04.08.2026)

> Патч-релиз поверх v1.1.1: четыре клиентские находки F45–F48, найденные и исправленные в PR #114, плюс серверный фикс F35 (§7.30), отложенный ещё с v1.1.0. Авария в документации: PR #114 был смержен без записей в roadmap.md — закрыта отдельным docs-онли PR #116 перед релизом.

| Событие | Коммит / ссылка | Комментарий |
|---|---|---|
| PR #114 — `fix(sync,logger)` | [PR #114](https://github.com/danny-swan/taskflow-app/pull/114), squash `0c2680b` | F45–F48 (§7.40–§7.43): несуществующее поле `._id` на таймере + `logger.ts` гасил детали `Error` в `meta: {}`; один необёрнутый блок `pinWorkspacePointers`/`reconcileLocalPlaceholder`; `sync_devices` upsert упирался в RLS при смене аккаунта на том же устройстве (`client_id` никогда не сбрасывался); `pullAll()` не ревалидировал сессию после старта. Нет миграций и изменений RLS — все четыре — клиентские фиксы |
| PR #115 — `fix(db)` | [PR #115](https://github.com/danny-swan/taskflow-app/pull/115), squash `38088fe` | F35 (§7.30): `REVOKE EXECUTE ON FUNCTION public.log_task_activity() FROM anon, authenticated, PUBLIC` — миграция `0041_revoke_execute_log_task_activity.sql`, тот же паттерн, что и в `0013`. Убирает два WARN из Security Advisor, триггер продолжает работать (выполняется от имени владельца таблицы) |
| PR #116 — `docs(roadmap)` | [PR #116](https://github.com/danny-swan/taskflow-app/pull/116), squash `23ee7b6` | Добавлены записи §7.40–§7.43 (F45–F48) — закрытие разрыва в документации после PR #114. В коде ничего не трогало |
| Миграция `0041` на прод | Supabase-коннектор, проект `sejpmzrmtgcvevukggkx`, 04.08.2026 ~21:00 UTC | Применена отдельно от тега релиза (чистая SQL-миграция, не часть Tauri-сборки). После применения проверено Security Advisor'ом повторно: оба WARN'а (`anon_security_definer_function_executable`, `authenticated_security_definer_function_executable`) для `log_task_activity` исчезли из списка |
| Релиз десктопа v1.1.2 | [тег v1.1.2](https://github.com/danny-swan/taskflow-app/releases/tag/v1.1.2), коммит `23ee7b6`, [CI run 30950508968](https://github.com/danny-swan/taskflow-app/actions/runs/30950508968) | Собраны Windows (NSIS + MSI RU/EN) и macOS (universal dmg), `latest.json` (`version: 1.1.2`) — юзеры v1.0.0–1.1.1 получат обновление через auto-updater. 90 файлов / 745 vitest, E2E Playwright, Deno-тесты Edge Functions зелёные. Релиз включает только F45–F48 (клиентские фиксы в сборке); F35 — серверная миграция, не входит в Tauri-артефакты напрямую, причём на проде уже применена (строка выше) |

**Статус F35 после v1.1.2:** миграция `0041` в main и на проде, WARN'ы Advisor'а исчезли — но статус в §7.30 остаётся “⚠️ готово, ждёт ручной проверки”, так как по правилу проекта статус “исправлено” ставится только после ручного дымового теста (смок-тест триггера логирования активности). F45–F48 — аналогично, ждут ручные шаги, перечисленные в §7.40–§7.43.
...
**✅ ИСПРАВЛЕНО (PR #106, 03.08.2026)** — подтверждено ручной проверкой пользователя на сборке с `main` (пункт 4 пост-merge чек-листа: смена аккаунта туда-обратно — задачи на месте, доска не пустая при непустом счётчике).
...
**✅ ИСПРАВЛЕНО (PR #106, 03.08.2026)** — подтверждено ручной проверкой пользователя на сборке с `main` (пункт 4 пост-merge чек-листа).
...
**✅ Обновление статуса — пост-merge чек-лист пройдён (03.08.2026, сборка с `main`, run `30808819642`):** пользователь подтвердил все пять пунктов ручной проверки: (1) создание и переключение пространств работает; (2) инвайт по TF-ID в новое пространство и совместная работа с задачами работают; (3) повторные приглашения на ту же пару работают (F15/реактивация, тест 20); (4) смена аккаунта туда-обратно (F31/F32); (5) восстановление при уже существующем personal-пространстве (F33). Единственная новая жалоба того же прогона — пустые «Задачи» на самом первом входе в свежий free-аккаунт → вынесена в отдельную находку F36 (§7.31).
...
### 7.30. `log_task_activity()` (миграция 0034) без `REVOKE EXECUTE` — SECURITY DEFINER функция доступна `anon`/`authenticated` через PostgREST RPC (F35, ✅ ИСПРАВЛЕНО (PR [#115](https://github.com/danny-swan/taskflow-app/pull/115), 04.08.2026), миграция `0041` применена на прод 04.08.2026, верифицировано агентом через Security Advisor — без ручной проверки пользователя, так как у находки нет UI-проявления)
...
**Применение и верификация на прод (04.08.2026):** PR #115 смержен в `main` (squash `38088fe`) после зелёного CI (type-check+unit, Edge Functions, E2E). Миграция `0041` применена на прод (проект `sejpmzrmtgcvevukggkx`) через Supabase-коннектор после `confirm_action`. Повторный сниток Security Advisor после применения подтвердил: оба целевые WARN'а (`anon_security_definer_function_executable`, `authenticated_security_definer_function_executable` для `log_task_activity`) исчезли из списка; остальные WARN'ы относятся к другим, намеренно открытым RPC (`accept_invite`, `invite_to_workspace` и т.д.), не к F35. Статус — исправлено.

---

### 7.31. Первый вход в свежий free-аккаунт: сид-статусы уезжали на `ws_local`, а welcome-задача — на `ws_<uid>` → «Задачи» пустые при целых данных (F36, ✅ ИСПРАВЛЕНО (PR #107, 03.08.2026), ручная проверка пройдена 03.08.2026)

**Симптом (сообщён пользователем при пост-merge проверке сборки с `main`):** при самом первом входе в свежий аккаунт с free-подпиской welcome-задача «есть, но не отображается» — страница «Задачи» пуста. После перехода в другой аккаунт и обратно отображение появляется.

**Корень — доказан НА РЕАЛЬНЫХ ДАННЫХ пользователя, не по гипотезе из кода.** Присланный `data-2.db-wal` по контрольным суммам совпал с `data-4.db` → оба файла сложены рядом как `data.db` + `data.db-wal`, sqlite применил WAL — и получилось живое состояние сломанного свежего аккаунта `1f797943-c8b5-4a3d-8e6b-c6dc7ac18c49` (всё создано 03.08.2026 11:39:10–11:39:11):

| Сущность | Состояние |
| --- | --- |
| `settings.current_workspace_id` = `personal_workspace_id` | `ws_1f797943c8b54a3d8e6bc6dc7ac18c49` (верно) |
| `workspaces` | 1 строка `ws_1f797943c8b54a3d8e6bc6dc7ac18c49` «Мои задачи», `personal`, 11:39:10 |
| `tasks` | 1 строка `019fc76b-af5d-7703-99f8-a4458c7f6d5d` «Добро пожаловать в TaskFlow», `status_id=2`, `workspace_id=ws_1f797943…` (верно), 11:39:11.069Z |
| `statuses` | **все 7 сид-статусов с `workspace_id='ws_local'`** (uuid `019fc76b-ae7c…`–`019fc76b-aed9…`, тот же миллисекундный интервал) |

Доска рендерит колонки по статусам **текущего** пространства → в `ws_<uid>` статусов нет → ни одной колонки → задаче негде показаться, хотя в БД целы и задача, и статусы.

**Механизм гонки (код на `main`, HEAD `72abd202`):**

1. `src/components/AccountSwitchGate.tsx` (free-ветка): `clearUserData()` → `setBoundUserId` → `reconcilePersonalWorkspace(userId)` → (нет слота/снимка) → `ensureSeededIfEmpty()` → `ensureWelcomeTaskIfNeeded(userId)`.
2. `clearUserData()` удаляет указатели `current_workspace_id`/`personal_workspace_id`, маркер `welcome_seeded` и все ws/статусы/теги/задачи.
3. `reconcilePersonalWorkspace` (`src/lib/sync/workspace.ts`) пишет указатели через синхронный `db.run()`. А в Tauri `db.run()` применяет SQL к web-зеркалу (sql.js) сразу, а к нативной SQLite — `getTauriDb().then(d => d.execute(...))`, то есть **fire-and-forget, без await** (`src/lib/db.ts` ≈строка 1514).
4. `ensureSeededIfEmpty()` в Tauri читала `personal_workspace_id` из **нативной** базы (`getTauriDb().select`) — запись шага 3 туда ещё не долетела → `seedWsId` падал на placeholder `'ws_local'` → 7 статусов и 5 тегов штамповались `ws_local`.
5. Через ~0,2 с `ensureWelcomeTaskIfNeeded()` читал уже долетевший указатель → задача с верным `ws_<uid>`. Расхождение внутри ОДНОГО bootstrap'а зафиксировано на диске.
6. Почему не помогли существующие механизмы: `repairTaskStatusWorkspaceMismatch()` (F32/ADR 0025) ищет одноимённый статус в целевом ws — там пусто; `reconcileLocalPlaceholder()` считал `ws_local` присутствующим только по `workspaces`/`workspace_members`, а там `ws_local` не было (personal-ws уже создан под `ws_<uid>`) → `hasLocalRefs=false` → сироты не переклеивались никогда. «Само починилось» при переходе между аккаунтами только потому, что `applyBackup`/restore слота перештамповывает `workspace_id` всех строк (лог: `restored slot for e46d9d68…: 2 tasks, 5 tags, 14 statuses`).

**Фикс (ADR 0028, без новых слоёв и без костылей поверх гонки):**

- `ensureSeededIfEmpty(seedWsIdOverride?)` и `ensureWelcomeTaskIfNeeded(_userId?, seedWsIdOverride?)` берут ws-id **из аргумента**; при переданном ws-id БД для указателя не читается вообще. Все три bootstrap-вызова передают `computeWorkspaceId(userId)`: `AccountSwitchGate.tsx` (free, `!restored`) и два места в `src/lib/sync/index.ts` (free/истёкший trial; досев после pull). Без аргумента — поведение прежнее.
- Статус для welcome-задачи ищется СТРОГО внутри целевого ws (`WHERE workspace_id=… AND deleted_at IS NULL`, приоритет «Сегодня», иначе минимальный `sort_order`); если статусов там нет — задача НЕ создаётся (`warn`), а не цепляется за чужой статус.
- Второй контур (self-heal уже испорченных баз, включая текущую `data.db` пользователя): в `reconcileLocalPlaceholder` (`src/lib/sync/workspace.ts`) признак `hasLocalRefs` расширен новым `hasLocalChildRefs()` по дочерним таблицам `LOCAL_WS_CHILD_TABLES` (`tasks`, `statuses`, `tags`, `task_templates`, `overdue_events`, `task_hold_periods`), и шаг переклейки идёт по тому же списку.
- Не тронуто: логика членства/инвайтов (ADR 0008/0009/0011), pro/облачный путь, слот free-аккаунта (ADR 0014/0021), файловые снимки (ADR 0023), схема БД — **новых миграций нет**.

**Верификация:** новый файл `src/lib/db.f36SeedWsStamp.test.ts` (4 кейса: штамп явным ws-id при ОТСУТСТВУЮЩЕМ указателе в settings; задача и её статус в одном ws; отказ от welcome, если статусы только в чужом ws; обратная совместимость без аргумента) + новый кейс в `src/lib/sync/workspaces-sync.test.ts` §(e) (переклейка сирот `ws_local`, когда строк `ws_local` в `workspaces`/`workspace_members` нет, + идемпотентность). Моки `../lib/db` и `../lib/sync/workspace` в `AccountSwitchGate.test.tsx` / `accountSwitchRestore.test.tsx` обновлены под новую сигнатуру (пробрасывают ws-id) + добавлен ассерт `ensureSeededIfEmpty` вызван с `ws_<uid>`. Полный прогон `vitest run`: **79 файлов / 658 тестов — все зелёные**; `tsc --noEmit` чист. Build локально НЕ запускался (OOM-ограничение среды) — только в CI. См. [ADR 0028](../adr/0028-seed-workspace-id-explicit.md).

**⚠️ Статус:** реализовано, тесты зелёные — ждёт ручную проверку пользователя (свежий free-аккаунт → «Задачи» показывают welcome-задачу сразу, без перехода между аккаунтами; старая испорченная база лечится при следующем запуске).

✅ **ИСПРАВЛЕНО (PR #107, 03.08.2026).** Ручная проверка пользователем пройдена 03.08.2026: на свежем free-аккаунте welcome-задача видна сразу, старая испорченная база вылечилась сама — в `taskflow-5.log` новой сборки видны строки самолечения `[sync/workspace] reconciled ws_local → ws_fc1930901c784a13855e4e9ecc52588d` (12:39:57, 12:40:03) и `… → ws_e46d9d68b253452a9bc1ac672e777c91` (12:43:25), а `app ready` отдаёт непустые `statuses/tasks`.

**Сопутствующая находка (заведена, чинится ОТДЕЛЬНО):** в `taskflow-3.log` ровно раз в минуту (11:35:00, 11:36:00, 11:38:00) падает `Uncaught TypeError: Cannot create property '_id' on number '36' / '129' / '324'` (`assets/index-Cvbp7k0V.js:762:38046`) — похоже на периодический таймер, передающий в библиотечный вызов число вместо объекта. На F36 не влияет. **Подтверждена в новом `taskflow-5.log` (сборка с фиксом F36):** та же ошибка ровно на минутных отметках 11:46:00, 11:52:00, 11:57:00, 12:40:00, 12:43:00 и в новом бандле `assets/index-ClzwNkSj.js:762:38046`, число в сообщении растёт (`36 → 129 → 324 → 561 → 1613 → 2667`) — похоже на счётчик тиков, уходящий в библиотечный вызов вместо объекта. Чинится ОТДЕЛЬНО, не смешивать с F36/F37.


---

### 7.32. Задача, созданная за секунды до выхода из free-аккаунта, могла пропасть: слот не писался при logout + записи в нативную SQLite шли без порядка и без точки ожидания (F37, ✅ ИСПРАВЛЕНО (PR #108, 03.08.2026), ручная проверка пройдена 03.08.2026)

**Симптом (сообщён пользователем 03.08.2026 после подтверждения F36):** «если создать задачу на free-аккаунте и быстро выйти из аккаунта, потом зайти в другой и снова вернуться, то задача может пропасть — видимо не успела сохраниться». Наблюдалось на аккаунте `e46d9d68-…` (`test4`), пространство «Мои задачи». Действие редкое, вероятность попадания низкая.

**Что доказано на данных, а что — по коду (честная граница).** В присланных `data.db` + `data-4.db-wal` + `taskflow-5.log` (1221 строка) **следов конкретной пропавшей строки нет и быть не может**: слот живёт в `localStorage` webview (в выгрузку не попадает), а `applyBackup(..., 'replace')` при восстановлении слота стирает таблицы. Поэтому корень доказан двумя другими измеримыми фактами:

1. **По логу — слот при обычном выходе не писался вовсе.** Все записи `[localAccountStore] saved slot …` в сессии идут либо через 30–48 с после мутаций (дебаунс автосейва: `12:41:30`, `12:42:08`, `12:43:59` — последняя через 48 с после предыдущего восстановления), либо непосредственно перед восстановлением слота другого аккаунта (то есть из `AccountSwitchGate`). Ни одной записи, привязанной к выходу из аккаунта, в логе нет.
2. **По коду — путей записи слота было ровно три, и logout в них не входил.** `useLocalAccountAutosave` пишет через `AUTOSAVE_DEBOUNCE_MS = 30_000` тишины; `beforeunload`/`visibilitychange` — только закрытие/сворачивание окна; `AccountSwitchGate` — слот уходящего при входе в другой аккаунт. `Settings.handleSignOut` вызывал лишь `flushOutboxBeforeLogout` (Fix 5), а тот по инварианту работает **только** для Pro/Trial (`if (!isProOrTrial(ent)) return`) — для free выход был «пустым».

Пока приложение живо, это ещё не потеря: Gate при входе в следующий аккаунт читает то же sql.js-зеркало и слот успевает записаться. Потеря возникает, если между выходом и возвратом процесс перезапускался — в этой же сессии рестарты были (`app start` в 12:38:23, 12:39:47, 12:40:03, 12:43:25; часть — рестарт после `[snapshots] restored … from file` в 12:39:58, ADR 0023). Тогда зеркало умирает вместе с процессом, а в файле задача есть только если fire-and-forget запись успела доехать.

**Второй, более общий корень — записи в нативную SQLite шли независимыми промисами.** `db.run()` в Tauri (db.ts ≈1543) применял SQL к зеркалу синхронно, а в файл отправлял `getTauriDb().then(d => d.execute(sql, params))` — отдельным промисом на каждую команду. Отсюда: (а) порядок применения не гарантирован — в пачке `clearUserData()` → `applyBackup()` быстрый `DELETE` мог лечь **после** медленного `INSERT`, и зеркало с файлом расходились; (б) точки ожидания не существовало — ни выход, ни `createSnapshot`, ни `restoreSnapshot` (подмена файла `data.db` через Rust) не могли дождаться незавершённых записей. Тот же fire-and-forget уже был корнем F18 (ADR 0012), F31 и F36 (ADR 0028) — там лечились его частные проявления.

**Фикс (ADR 0029):**

- `src/lib/db.ts` — одна последовательная очередь `nativeWriteChain` (`enqueueNativeWrite`) вместо промиса на команду: записи доезжают до файла **в порядке вызова**; ошибка звена ловится внутри и не рвёт цепочку. Контракт `run()` не изменился — он по-прежнему синхронный. Новый экспорт `flushNativeWrites()` — точка ожидания (в web no-op).
- `src/lib/snapshots.ts` — `await db.flushNativeWrites()` перед `invoke('snapshot_db')` (снимок содержит последние мутации) и перед `invoke('restore_snapshot')` (команды «в полёте» не применяются к уже подменённому файлу).
- `src/lib/logoutLocalSlot.ts` (новый) — `saveFreeSlotBeforeLogout(userId, email)`: no-op при чужом `bound_user_id` и для Pro/Trial/Lifetime (инвариант ADR 0014 сохранён), иначе `saveLocalAccountData` + `flushNativeWrites`; никогда не бросает. Вызов — в `Settings.handleSignOut` перед `flushOutboxBeforeLogout` и `signOut()`.

Схема БД и миграции не затронуты (новых нет), логика членства/инвайтов (ADR 0008/0009/0011), облачный путь Pro/Trial, F36 (ADR 0028) и форматы слота/снимков — тоже.

**Верификация:** новый `src/lib/db.nativeWriteQueue.test.ts` (4 кейса на реальном Tauri-пути `db.ts` поверх sql.js-адаптера с искусственной задержкой; **без фикса 2 из 4 падают** — проверено откатом), новый `src/lib/logoutLocalSlot.test.ts` (6 кейсов). Полный прогон `vitest run`: **81 файл / 668 тестов — все зелёные**; `tsc --noEmit` чист. Сборка — только в CI.

**Статус:** ✅ **ИСПРАВЛЕНО (PR #108, 03.08.2026).** Ручная проверка пользователем пройдена 03.08.2026: задача, созданная за 2–3 с до выхода, остаётся на месте и при возврате из другого аккаунта, и при перезапуске приложения между выходом и возвратом; создание задачи с последующим немедленным закрытием приложения тоже сохраняется. **Отложено отдельным пунктом проверки (не блокирует находку):** сценарий «Настройки → Хранилище» — создание снимка и восстановление из снимка (пользователь не проверял, т.к. не пользовался этим механизмом). Проверить после остальных пунктов плана; регресс на этот путь закрыт тестами `db.nativeWriteQueue.test.ts` (ожидание очереди перед `snapshot_db`/`restore_snapshot`).


---

### 7.33. При входе в аккаунт и при смене пространства активная вкладка не сбрасывалась на «Вкладку по умолчанию» (F38, ⚠️ реализовано, тесты зелёные, ждёт ручную проверку, 03.08.2026)

> Приписка 03.08.2026: ✅ ручная проверка пройдена 03.08.2026 (вошло в релиз v1.1.0).

**Симптом (пункт 3 брифа, два проявления):** (1) при переключении пространства пользователь остаётся на текущей вкладке, хотя ожидается «Вкладка по умолчанию» из «Настройки → Основные»; (2) «иногда при входе в аккаунт я оказываюсь во вкладке Настройки, а не на Задачах».

**Корень — доказан по коду и по фактическому поведению роутера (не по гипотезе).**

1. **Адрес не сбрасывается при выходе.** Приложение работает под `HashRouter` (`src/main.tsx`), выход делается из «Настроек» (`Settings.handleSignOut`). После `signOut()` `App` рендерит `AuthScreen` **вместо** `<Routes>`, но сам маршрут остаётся `#/settings`; вход возвращает `<Routes>` — и пользователь снова в «Настройках». Отсюда «иногда»: при выходе с «Задач» симптома нет.
2. **`switchWorkspace()` не трогает маршрут.** `store.switchWorkspace()` (`src/store/useStore.ts`) меняет `currentWorkspaceId`, персистит `current_workspace_id`, делает `refresh()` и переподписывает realtime — навигации там нет (и по слоям быть не должно: стор не знает про роутер). Точек смены пространства пять: `WorkspaceSwitcher`, `MyInvitesSection`, `CreateWorkspaceModal` и две автоматические ветки внутри `init()`/reconcile.

**Фикс (ADR 0030):** новый `src/lib/useDefaultTabNavigation.ts` — одна точка, вызываемая в `App`: сравнивает «кто вошёл» и «какое пространство активно» с предыдущими значениями и при изменении переводит на `/<defaultTab>` (`replace: true`). На первом проходе намеренно молчит (маршрут при старте уже разрулен `/` → `/<defaultTab>`, а deep-link перебивать нельзя); до `ready` не работает; при отсутствии сессии не навигирует, но запоминает `null`, поэтому вход распознаётся даже при возврате в тот же аккаунт. Навигацию в каждое из пяти мест смены ws не дублируем — это разъехалось бы.

**Компромисс (осознанный):** если пространство меняется автоматически (reconcile после pull, когда текущее ws исчезло), пользователя тоже переведёт на дефолтную вкладку. Редкая ветка, и это лучше, чем остаться на вкладке несуществующего пространства.

**Верификация:** `src/lib/useDefaultTabNavigation.test.tsx` — 8 кейсов (первый проход молчит; смена ws → переход; `defaultTab` читается из настроек, а не хардкод; выход и вход в тот же аккаунт → переход; вход в другой аккаунт → ровно один переход; `ready=false` не запоминает состояние; `workspaceId=null` не навигирует; повторные рендеры без изменений не навигируют). Полный прогон: **84 файла / 685 тестов зелёные**, `tsc --noEmit` чист.

**Статус:** ⚠️ реализовано, ждёт ручной проверки: переключить пространство с любой вкладки (например со «Статистики») → должно открыться на дефолтной вкладке; выйти из аккаунта, находясь в «Настройках», и войти снова → «Задачи»; сменить аккаунт → «Задачи»; принять приглашение → дефолтная вкладка.

---

### 7.34. Переходы в конкретные секции «Настроек» не работали: секция читалась из `window.location.hash`, который под HashRouter содержит весь маршрут (F39, ⚠️ реализовано, тесты зелёные, ждёт ручную проверку, 03.08.2026)

> Приписка 03.08.2026: ✅ ручная проверка пройдена 03.08.2026 (вошло в релиз v1.1.0).

**Симптом (пункт 9 брифа):** (1) клик по trial-баннеру должен вести в «Настройки → Подписка», но открываются «Основные»; (2) клик по «Доступно обновление… Открыть» должен вести в «Настройки → Обновления», а ведёт в «Основные».

**Корень — доказан фактическим поведением роутера в тесте, а не чтением кода.** Приложение под `HashRouter`, то есть **весь маршрут лежит в `window.location.hash`**. `Settings.tsx` определял секцию так: `window.location.hash === '#subscription'`. После `navigate('/settings#subscription')` реальный адрес — `#/settings#subscription`, поэтому `window.location.hash` равен `'#/settings#subscription'` и **никогда** не равен `'#subscription'` — условие не выполнялось ни разу. Все три вызывающих места (`SubscriptionBanner` в `src/components/Sidebar.tsx`, две кнопки в `src/components/PaywallModal.tsx`) вели на верный адрес, но открывались «Основные». Второй слой — слушатель `hashchange` — тоже не спасал: react-router меняет адрес через `pushState`/`replaceState`, а они событие `hashchange` не генерируют. Тост обновления вёл вообще без секции (`navigate('/settings')`).

Оба значения зафиксированы тестом на живом `HashRouter` (`src/lib/settingsHashRoute.test.tsx`): `window.location.hash === '#/settings#subscription'`, `useLocation().hash === '#subscription'`.

**Побочно найдено в том же слое и починено:** `src/pages/Checkout.tsx` использовал `<a href="/settings">` — под HashRouter это уход на несуществующий реальный путь (в Tauri — пустой экран вместо настроек).

**Фикс (ADR 0030):** новый `src/lib/settingsSections.ts` (список ключей секций + чистая функция `parseSettingsSection`, понимающая и `'#updates'`, и полный `'#/settings#updates'`; неизвестное → `null`). В `Settings.tsx` источник — `useLocation().hash` из react-router (одинаково работает в HashRouter и BrowserRouter); **оба сломанных слоя убраны целиком** — и сравнение с `window.location.hash`, и слушатель `hashchange`, костыля поверх нет. `App.tsx`: тост обновления → `/settings#updates`, возврат с успешной оплаты (`taskflow://pay/success`) → `/settings#subscription`. `Checkout.tsx`: ссылка → кнопка с `navigate('/settings#subscription')`. Trial-баннер и `PaywallModal` не менялись — их адрес наконец работает.

**Верификация:** `src/lib/settingsSections.test.ts` (7 кейсов на разбор хэша, включая полный маршрутный хэш и все ключи секций) + `src/lib/settingsHashRoute.test.tsx` (2 кейса на живом роутере: доказательство корня и переключение секции на уже открытой странице). Полный прогон: **84 файла / 685 тестов зелёные**, `tsc --noEmit` чист.

**Статус:** ⚠️ реализовано, ждёт ручной проверки: клик по trial-баннеру → «Настройки → Подписка»; клик по «Доступно обновление… Открыть» → «Настройки → Обновления»; то же с уже открытой страницей настроек (секция должна переключаться); на `/checkout` ссылка «← Вернуться в настройки» → «Настройки → Подписка».

---

### 7.35. Во вкладке «Участники» вместо имени показывался кусок внутреннего uuid, а аватары у всех выглядели одинаковыми (F40, ⚠️ реализовано, тесты зелёные, ждёт ручную проверку, 03.08.2026)

> Приписка 03.08.2026: ✅ ИСПРАВЛЕНО (PR #110, 03.08.2026), миграции 0042/0043 применены на прод 03.08.2026, ручная проверка пройдена 03.08.2026.

**Симптом (пункт 5 брифа):** «участники ws показывают кусок uuid», нужно показывать TF-id либо ник, если он задан; отдельно — «в настройках пространства отображались реальные аватары пользователей, а то такое ощущение, что там одинаковые аватары».

**Корень — доказан на проде SQL-запросами, а не чтением UI.**

1. Имя строилось как `m.user_id.slice(0, 8)` (`src/components/MembersTab.tsx`) — первые 8 символов внутреннего uuid.
2. Это не «недоделка вида», а следствие политик доступа: на `profiles` есть только own-row политики `profiles_select_own` / `profiles_update_own` с условием `auth.uid() = id`. Клиент **физически не может** прочитать `nickname` / `public_user_id` / `avatar_variant` другого пользователя — SELECT по чужому uuid возвращает 0 строк. Поэтому и аватар рисовался вариантом по умолчанию (`variant = 1`) для всех, кроме тех, кто прямо сейчас онлайн: у онлайн-участников данные приходят не из `profiles`, а из presence-meta (`src/lib/presence.ts`).
3. Проверка списка функций на проде (`sejpmzrmtgcvevukggkx`) показала прецедент выдачи публичного минимума — `find_user_by_public_id(p_pid text)` (миграция `0028`, поиск по TF-id для инвайтов), — и **отсутствие** обратной функции «по членству → профили». Прод-данные для верификации: `TF-B68WC7` (ник пустой, аватар 1), `TF-2YGP43` («Admin», аватар 8), `TF-7EDA2R` («Daniil Lebedev», аватар 6), `TF-5EN7DP` (ник пустой, аватар 1).

**Фикс (миграция `0043`; ADR 0031).** Новая SECURITY DEFINER STABLE RPC `public.get_workspace_member_profiles(p_workspace_id text)`: гейты `auth.uid() is not null` + `has_workspace_role(p_workspace_id, auth.uid(), 'viewer')` (вызывающий сам обязан быть участником) + `deleted_at is null` (вышедшие не отдаются); возвращает строго `(user_id, public_user_id, nickname, avatar_variant, avatar_color, bio)` — `email` в сигнатуре отсутствует; `REVOKE` для `anon`/`PUBLIC`, `GRANT EXECUTE` для `authenticated` (паттерн ADR 0002). RLS на `profiles` НЕ ослаблялась: политика открыла бы строку целиком вместе с email. Клиент — новый `src/lib/memberProfiles.ts` (вызов RPC, нормализация, кэш `localStorage['tf.memberProfiles.v1']`, правило «есть ник → ник, иначе TF-id» в одной функции `memberDisplayName`); `MembersTab` показывает «вы» / ник / TF-id / нейтральное «Участник» и реальные аватары участников; клик по имени открывает новую карточку `MemberInfoModal` (аватар, ник, роль, «о себе», TF-id — без email и без внутреннего uuid); в `ActivityEntry` фолбэк `record.userId.slice(0, 8)` **удалён**, порядок резолва автора стал «я → presence → кэш публичных профилей → «Участник»».

**Что НЕ трогали (по условию брифа):** логику членства и приглашений (ADR 0008/0009/0011, `accept_invite`, RLS `sync_workspace_members`) — новая функция только читает; sync-цикл — `profiles` в него не входит; политики `profiles` остались own-row.

**Верификация:** pgTAP `supabase/tests/21_member_profiles_test.sql` (`plan(13)`): колонка `avatar_color` и её nullable, гранты (`anon` — нет, `authenticated` — да), `prosecdef`, отсутствие `email` в сигнатуре, CHECK-формат цвета, участник видит двух живых участников с TF-id/ником/цветом, вышедший (`deleted_at`) исключён, не-участник получает 0 строк. vitest: `src/lib/memberProfiles.test.ts`, `src/components/MemberInfoModal.test.tsx`, обновлённые `MembersTab.test.tsx` (ник → TF-id → «Участник», аватар с цветом, открытие карточки) и `TaskActivityLog.test.tsx` (uuid в разметке отсутствует). Полный прогон: **87 файлов / 727 тестов зелёные**, `tsc --noEmit` чист.

**Прод-проба под ROLLBACK (03.08.2026, `sejpmzrmtgcvevukggkx`).** В транзакции применены `0042`+`0043`, профилю `TF-2YGP43` выставлен `avatar_color='#ff8800'`, затем `set_config('request.jwt.claims', …)` + `set local role authenticated` от имени участника `9ef5d96b…` (`TF-B68WC7`, роль viewer в `ws_019fc774040379d98aa283e34148d52d`): функция вернула 2 строки — `TF-B68WC7` (ник null, аватар 3, цвет null) и `TF-2YGP43` («Admin», аватар 8, цвет `#ff8800`, bio «TaskFlow Developer»). Повторная проба от имени **не-участника** (`TF-7EDA2R`) вернула `0` строк. Транзакции откачены — состояние прода не изменено.

**Статус:** ⚠️ реализовано, ждёт ручной проверки: открыть «Настройки → Участники» в общем пространстве → у каждого участника ник либо TF-id (не цифры) и свой аватар; клик по имени → карточка с ником/аватаром/«о себе»/TF-id; в «Истории» пространства авторы прошлых действий тоже без куска uuid.

---

### 7.36. Цвет аватара был производной темы приложения, а не выбором пользователя; блок «Аккаунт» шёл в неудобном порядке (F41, ⚠️ реализовано, тесты зелёные, ждёт ручную проверку, 03.08.2026)

> Приписка 03.08.2026: ✅ ИСПРАВЛЕНО (PR #110, 03.08.2026), миграции 0042/0043 применены на прод 03.08.2026, ручная проверка пройдена 03.08.2026.

**Симптом (пункт 6 брифа):** нужен редизайн «Настройки → Аккаунт» (профиль сверху: ник слева, аватар справа, выбор аватара — во всплывающем окне, ниже «О себе» и «Ваш ID», под ними блок email/«Зарегистрирован»/«Последний вход») и **явный выбор цвета** аватара: «цвет аватарки задаётся явно и НЕ меняется автоматически вместе с темой приложения (сейчас он привязан к теме — это убрать)». Уточнение по цвету — «любой цвет (пикер)», и «чтобы у всех отображался тот цвет, который пользователь себе сам задал».

**Корень (по коду, подтверждён видом в двух темах).** `Avatar` красился классами `text-accent` + `bg-surface-alt`, то есть цвет брался из CSS-переменных активной темы. Следствия: (1) при смене темы аватар менял цвет; (2) в общих пространствах цвет не мог различаться между участниками в принципе — даже когда форма аватара известна (онлайн-участники через presence), все глифы красились акцентом **моей** темы, отсюда «ощущение, что там одинаковые аватары». Цвет не хранился нигде: колонки под него в `profiles` не было.

**Фикс (миграция `0042`; ADR 0032).** `profiles.avatar_color text` (nullable) + CHECK `profiles_avatar_color_format` (`~* '^#[0-9a-f]{6}$'` или NULL) — формат проверяется на сервере, `NULL` = «цвет не задан» и даёт прежнее поведение (обратная совместимость со старыми профилями и старыми сборками). `Avatar` при явном цвете ставит inline `color` и подложку через новый `avatarTint(color, 0.14)`, посчитанный **ручным `rgba()` без CSS `color-mix`** (клиент работает в системном WebView Tauri, поддержка `color-mix` там не гарантирована); невалидное значение игнорируется — падаем на тему, а не рисуем сломанный аватар. Выбор формы и цвета вынесен в модалку `AvatarEditorModal`: превью **одновременно на светлой и тёмной подложке**, сетка форм, системный `input type="color"` + hex-поле с валидацией и подсказкой «Формат: #rrggbb» + 10 быстрых свотчей; «Готово» отдаёт значения наружу, «Отмена»/Esc не меняют ничего. Блок «Профиль» в `AccountSection` перестроен (ник + поле ввода слева, аватар 64px и кнопка «Изменить» справа на том же уровне, ниже «О себе» со счётчиком, ниже «Ваш ID» с копированием) и перенесён **выше** блока email/«Зарегистрирован»/«Последний вход». Цвет доезжает до других людей двумя путями: своим профилем (`PROFILE_COLUMNS`) и публичной RPC `get_workspace_member_profiles` (F40), а для онлайн-участников — опциональным полем `avatar_color` в presence-meta (клиент прошлой версии его просто не пришлёт).

**Верификация:** vitest — `Avatar.test.tsx` (тема при `color=null`, inline-цвет при явном значении, игнор мусора, `avatarTint`), новый `AvatarEditorModal.test.tsx` (гидрация, дефолт, свотчи, системный пикер, валидный/невалидный hex, «Отмена» ничего не применяет, двойное превью), `ProfileBlock.test.tsx` (порядок «ник → о себе → Ваш ID», выбор в модалке, сохранение с `avatar_color`), `presence.test.ts` (цвет в meta), `PresenceIndicator.test.tsx`, `usePresenceStore.test.ts`; pgTAP `21` (колонка + CHECK отклоняет `'red'` с 23514 + цвет доезжает через RPC). Полный прогон: **87 файлов / 727 тестов зелёные**, `tsc --noEmit` чист.

**Статус:** ⚠️ реализовано, ждёт ручной проверки: «Настройки → Аккаунт» — сверху «Профиль», под ним email/«Зарегистрирован»/«Последний вход»; кнопка «Изменить» открывает окно выбора аватара; выбрать цвет пикером и «Готово» → «Сохранить профиль»; переключить тему приложения → цвет аватара НЕ меняется; в общем пространстве участники видят выбранные друг другом цвета.

---

### 7.37. «Покинуть пространство» не доезжало до сервера: soft delete уходил PostgREST-upsert'ом, а INSERT-политика RLS разрешена только владельцу (F42, ⚠️ реализовано, тесты зелёные, ждёт ручную проверку, 03.08.2026)

**Симптом (сообщение пользователя 03.08.2026).** Аккаунт `lebedevdo.one+test1@gmail.com` (роль editor, приглашённый) вышел из общего пространства «123» — у него пространство пропало. Но в аккаунте владельца `lebedevdo.one@gmail.com` тот же участник продолжал числиться в «Настройки → Участники». А при повторном входе в аккаунт test1 пространство «123» появилось у него снова.

**Корень (доказан на прод-данных, не по коду).**

1. **Выхода на сервере не было вообще.** В `public.sync_workspace_members` строка `(ws_019f67531f5b750ca62574e508d2116c, TF-B68WC7)` живая: `deleted_at is null`, `updated_at = 2026-07-22 12:48:38+00` — это дата приглашения, а не дата выхода. То есть гашение не долетало ни разу, и «участник висит у владельца» — не подвисший кэш, а фактическое состояние базы.
2. **Почему не долетало.** `pushBatch` отправлял **любую** операцию, включая `op='delete'`, через `supabase.from(...).upsert(rows, { onConflict })`. PostgREST-upsert — это `INSERT ... ON CONFLICT DO UPDATE`, а Postgres проверяет INSERT-политику RLS **всегда**, даже когда фактически исполняется ветка `DO UPDATE`. Политика `sync_workspace_members_insert_ws_role` разрешает INSERT только владельцу пространства, поэтому у editor'а запрос отбивался. Прод-проба под ROLLBACK от имени самого пострадавшего пользователя (`TF-B68WC7`, editor в этом ws), в одной транзакции: путь `INSERT ... ON CONFLICT (id) DO UPDATE SET deleted_at=…` → `42501 / new row violates row-level security policy for table "sync_workspace_members"`; путь `UPDATE … SET deleted_at=now() WHERE id=…` → `OK`, погашена 1 строка. Нужная политика на сервере **уже была** — `sync_workspace_members_self_leave_update` (`USING user_id = auth.uid() AND role <> 'owner'`, `WITH CHECK … AND deleted_at IS NOT NULL`), но она работает только для честного UPDATE.
3. **Почему тихо.** `42501` справедливо классифицируется в `isPermanentError` как permanent — строка оставалась в `sync_outbox` с `last_error` и выпадала из push'а. Пользователю ничего не показывалось.
4. **Почему пространство возвращалось.** Фаза 1 пула тянет своё членство `full: true` на каждом старте (F14) — живая серверная строка восстанавливала локально погашенную вместе с пространством. Владельцу та же живая строка приходила проходом B (`workspace_id IN (мои ws)`), отсюда «висит в участниках».

Отдельно: шапка `push.ts` с самого начала описывала правильный контракт («`.update({deleted_at: now()})` для `op='delete'`») — разошлась именно реализация, о чём честно сообщал комментарий рядом с кодом отправки.

**Фикс (ADR 0033, без миграций и без изменения RLS).** `op='delete'` уходит `UPDATE`'ом по ключу строки: новый `softDeleteRow(cloud, row, onConflict)` берёт ключ из спека таблицы (`id`, а для `workspace_settings` — составной `workspace_id,key`) и шлёт **узкий** патч `SOFT_DELETE_FIELDS = deleted_at, updated_at, version, client_id` — гашение не переписывает содержимое строки (роль участника, заголовок задачи) и не задевает `WITH CHECK` политик. Delete-группа обрабатывается построчно (один UPDATE на строку, свой вердикт у каждой), нет строки в облаке → UPDATE меняет 0 строк и это успех (гасить нечего), нет значения ключа в payload → внятная ошибка без запроса. Путь `op='upsert'`, `PUSH_ORDER` (upsert раньше delete в батче), backoff, классификация ошибок, логика членства/приглашений (ADR 0008/0009/0011) и схема БД **не изменены**. Альтернативы «разрешить editor'у INSERT в RLS» и «завести RPC `leave_workspace()`» отклонены (расширение прав / лишний слой поверх работающей политики) — см. ADR 0033.

**Верификация:** `src/lib/sync/push.test.ts` — 20 тестов, из них новые якоря F42: гашение уходит `update`'ом (`upsertCalls.length === 0`) с фильтром по ключу, патч содержит **только** поля гашения, ошибка UPDATE → `failed` + строка остаётся в outbox с `attempt_count=1`, составной ключ `workspace_id,key`, отсутствующий ключ → ошибка без запроса. Все 9 файлов `src/lib/sync` (97 тестов) зелёные. pgTAP `supabase/tests/22_member_self_leave_rls_test.sql` (`plan(6)`, подключён в `db-tests.yml`): upsert-путь у editor'а падает с 42501, UPDATE-путь проходит, строка гасится, поднять себе роль нельзя, чужое членство погасить нельзя, политика self-leave исключает `owner`.

**Известное ограничение (осознанное).** Записи в локальном outbox, уже сгоревшие на старом коде (`attempt_count >= 5`, `last_error` = 42501), фикс не оживляет — миграция локальной очереди была бы сложнее самого фикса. После обновления достаточно выйти из пространства ещё раз.

**Статус:** ⚠️ реализовано, ждёт ручной проверки: под test1 выйти из пространства «123» → у владельца участник исчезает из «Настройки → Участники»; повторный вход под test1 — пространство «123» больше не возвращается; приглашение и повторное принятие по-прежнему работают.

> Приписка 03.08.2026: ✅ ИСПРАВЛЕНО (PR #111, 03.08.2026), ручная проверка пройдена 03.08.2026 — выход из пространства доезжает до владельца и не возвращается при повторном входе.

---

### 7.38. Пользовательские тексты отстали от продукта: «Канбан» вместо «Карточки», FAQ и онбординг ничего не знали о пространствах (F43, ⚠️ реализовано, тесты зелёные, ждёт ручную проверку, 03.08.2026)

**Симптом (пункты 4, 7, 8 брифа).** (4) В интерфейсе второй вид вкладки «Задачи» назывался «Канбан» — слово для пользователя непонятное, нужно «Карточки». (7) FAQ во вкладке «Помощь» описывал приложение до волны пространств: ни слова про workspaces, приглашения по TF-id, роли, профиль и аватар; часть ответов («где хранятся задачи», «как работает синхронизация», «вошёл под другим аккаунтом») была верна только для одного личного пространства. (8) Онбординг из 10 шагов тоже не упоминал пространства и звал второй вид «Kanban».

**Корень.** Не баг кода, а рассинхрон текстов с продуктом: волна `feat/workspaces` (PR #74–#106) и последующие F36–F41 меняли поведение, а пользовательская документация внутри приложения обновлялась не всегда. Дополнительно нашлись две фактические ошибки в текстах: в онбординге перечислялись темы «Akatsuki, Konoha», хотя в `i18n.ts` темы называются «Акацуки»/«Деревня листа» (`Akatsuki`/`Hidden Leaf`), а в FAQ была фраза о «передаче владения» пространством — такой функции в коде нет (владелец может только удалить пространство целиком через `deleteWorkspace`).

**Фикс (только видимый текст, без изменения логики; ADR не требуется — архитектурного решения нет).**

- **Пункт 4:** `src/pages/Tasks.tsx` — заголовок и подпись переключателя «Канбан»/«Kanban» → «Карточки»/«Cards»; `src/lib/i18n.ts` — `action_toggle_view` → «Переключить вид (список/карточки)» / `Toggle view (list/cards)`. Имена компонентов и файлов (`KanbanBoard`, `KanbanColumn`, `KanbanCard`), значение `tasksView === 'kanban'` и записи в `changelog.ts` **намеренно не трогали** — это внутренние идентификаторы и история, которую не переписываем.
- **Пункт 7 (`src/pages/Help.tsx`):** новый раздел «🗂 Пространства и участники» / «🗂 Workspaces & members» из 8 вопросов (что такое пространство и лимиты 2 личных на free / 7 суммарно на платном; переключатель и группы «Личные/Общие»; «Вкладка по умолчанию» — F38; приглашение по `TF-XXXXXX` и экран «Мои приглашения»; роли owner/editor/viewer; «Покинуть пространство» и удаление участника владельцем; TF-id и ник с фолбэком в списках — F40; профиль и `AvatarEditorModal` с явным цветом — F41). Обновлены три существующих ответа (хранение, синхронизация, вход под другим аккаунтом) — добавлено поведение пространств и явно сказано, что на free-плане синхронизации нет вообще. Убрана несуществующая «передача владения». Блок «Поддержать разработчика» (`SupportBlock`) и секция горячих клавиш **не тронуты** (клавиши сверены с `App.tsx`, расхождений нет).
- **Пункт 8 (`src/components/Onboarding.tsx`):** механика spotlight, тип `Step`, публичные экспорты и `data-onboarding`-селекторы не менялись. Добавлен один шаг «Пространства»/«Workspaces» вторым по счёту (`target: null`, без подсветки — у `WorkspaceSwitcher` нет `data-onboarding`-атрибута, добавлять его ради подсветки не стали), шагов стало 11. Исправлены названия тем.

**Верификация:** новые `src/pages/Help.test.tsx` (5 кейсов: раздел про пространства есть в RU и EN, в текстах FAQ нет «Канбан»/«Kanban», роли описаны как owner/editor/viewer без несуществующего «admin») и `src/components/Onboarding.test.tsx` (3 кейса: контракт `isOnboardingSeen`/`markOnboardingSeen`/`resetOnboarding`, автозапуск показывает «Шаг 1 / 11», проход по всем 11 шагам на обоих языках без слова «Канбан»). Проверка на «Канбан» в Help намеренно ограничена FAQ-аккордеонами: `WhatsNewSection` рендерит историю `changelog.ts` (v0.9.34), где слово остаётся — историю не переписываем.

**Статус:** ⚠️ реализовано, ждёт ручной проверки: на вкладке «Задачи» переключатель называется «Карточки»; «Помощь» → раздел «🗂 Пространства и участники» на месте и читается; «Настройки → Общие» → сброс онбординга (или новый профиль) → в онбординге 11 шагов, второй — «Пространства», в шаге про задачи слово «Карточки».

> Приписка 03.08.2026: ✅ ИСПРАВЛЕНО (PR #111, 03.08.2026), ручная проверка пройдена 03.08.2026. По итогам проверки пользователь снял четыре замечания к текстам и один пробел в доступе — они закрыты отдельно, см. §7.39 (F44).

---

### 7.39. Наблюдатель не мог выйти из общего пространства: единственный вход на экран настроек пространства был закрыт гейтом `canEdit`; плюс правки текстов по итогам ручной проверки (F44, ⚠️ реализовано, тесты зелёные, ждёт ручную проверку, 03.08.2026)

**Симптом (ручная проверка пользователя 03.08.2026).** «У пользователей, у кого в пространстве роль Наблюдатель, нет возможности выйти из пространства, так как нет кнопки с настройками». Плюс четыре замечания к текстам, добавленным в F43: (1) в FAQ раздела «Пространства» лишний вопрос про «Вкладку по умолчанию»; (2) первый шаг тура утверждает «Работает офлайн, все данные хранятся локально в SQLite» — неполно, синхронизация на платном тарифе есть; (3) шаг «Пространства» не подсвечивает сам переключатель; (4) шаг «Помощь» ссылается на несуществующую кнопку «Пройти тур заново»; (5) финальный шаг говорит «Синхронизация с облаком (Supabase) — опциональная, включается на экране входа», хотя фактически она определяется тарифом, а офлайн-работа сохраняется и с подпиской.

**Корень (доказан на прод-политиках, серверная часть исправна).** Кнопка «Покинуть пространство» живёт во вкладке «Участники» экрана `/workspace-settings`, а единственная ссылка на этот экран — в меню переключателя пространств — была обёрнута гейтом `{canEdit && …}` с комментарием «только editor+ (viewer не видит)». Для роли viewer `useCanEdit()` возвращает `false`, поэтому входа на экран не было вообще, а вместе с ним и выхода из пространства: наблюдатель оказывался заперт. На сервере при этом всё разрешено — политики прод-БД: `sync_workspace_members_select_ws_role` (USING `has_workspace_role(..., 'viewer') OR user_id = auth.uid() …`) даёт viewer'у читать список участников, а `sync_workspace_members_self_leave_update` (USING `user_id = auth.uid() AND role <> 'owner'`) разрешает гашение своей строки любому не-владельцу, то есть и наблюдателю. Проблема была чисто в UI-гейте.

**Фикс (ADR [0034](../adr/0034-workspace-settings-visible-to-all-members.md)).**

- Ссылка «Настройки пространства» показывается **всем участникам**, гейт `canEdit` снят (вместе с неиспользуемым хуком). Отдельный урезанный экран для viewer'а не заводили: страница уже read-only для не-владельца — справочники статусов/тегов/дедлайнов гасятся `useCanManageWorkspace()`, переименование и удаление пространства доступны только owner'у, а `MembersTab` по своему контракту («editor/viewer: только список + Покинуть пространство») рисует наблюдателю ровно то, что ему положено. Это ровно «убрать сломанный слой», а не строить второй экран поверх.
- Тексты: из FAQ убран вопрос про «Вкладку по умолчанию»; первый шаг тура переписан («задачи хранятся локально в SQLite и доступны без интернета; на платном тарифе добавляется синхронизация между устройствами и общие пространства»); шаг «Пространства» получил `target: 'workspace-switcher'` — в `WorkspaceSwitcher` добавлен якорь `data-onboarding="workspace-switcher"`, теперь переключатель подсвечивается spotlight'ом; шаг «Помощь» ссылается на реальный вопрос «Как перезапустить приветственный тур?» вместо несуществующей кнопки; финальный шаг честно описывает модель («без входа и на бесплатном тарифе всё остаётся на устройстве; на платном — синхронизация через облако, при этом работа офлайн сохраняется, изменения уходят при появлении сети»).

**Верификация:** `WorkspaceSwitcher.test.tsx` — 3 новых кейса (viewer видит ссылку на настройки пространства и её `href`; владелец видит её по-прежнему; корневой контейнер помечен `data-onboarding="workspace-switcher"`). Существующие `Help.test.tsx` и `Onboarding.test.tsx` зелёные (тур по-прежнему 11 шагов). Политики прод-БД прочитаны напрямую (список выше) — серверных изменений не потребовалось, миграций нет.

**Статус:** ⚠️ реализовано, ждёт ручной проверки: под аккаунтом с ролью «Наблюдатель» открыть переключатель пространств → «Настройки пространства» → вкладка «Участники» → «Покинуть пространство» (работает, а справочники и переименование остаются недоступны); в туре второй шаг подсвечивает переключатель пространств; в FAQ раздела «Пространства» больше нет вопроса про «Вкладку по умолчанию».

---

### 7.40. `Topbar.tsx` писал в несуществующее поле `._id` на таймере, а `logger.ts` гасил детали любой ошибки в `meta: {}` (F45, ✅ ИСПРАВЛЕНО (PR [#114](https://github.com/danny-swan/taskflow-app/pull/114), 04.08.2026), выпущено в [v1.1.2](https://github.com/danny-swan/taskflow-app/releases/tag/v1.1.2) — нет UI-проявления, без ручной проверки)

**Корень.** `Topbar.tsx` протаскивал id интервала из `setTimeout`-колбэка через `(timeout as any)._id = id`, а читал обратно тем же нетипизированным хаком. Возвращаемое `setTimeout` не имеет поля `._id` — запись выполнялась без ошибки (JS позволяет навесить произвольное свойство на любой объект), но смысл уже был сломан — это не способ хранить id интервала, просто он случайно не бросался. Найдено при трассировке того, почему записи `[sync/orchestrator] reconcilePersonalWorkspace failed` несли `meta: {}` без полезных деталей: `logger.ts` сериализует `meta` через `JSON.parse(JSON.stringify(meta))`, а `JSON.stringify` на объекте `Error` выбрасывает `message`/`stack`/`name` (они не-перечисляемые) — всегда возвращает `'{}'`, какой бы ошибка ни была.

**Фикс.** `Topbar.tsx` — интервал id теперь ведёт через обычную захватываемую переменную `let intervalId`, без чужого поля. `logger.ts`: `write()` теперь отдельно обрабатывает случай `meta instanceof Error` и берёт `name`/`message`/`stack` напрямую, до падения в прежний JSON-путь для всего остального. Фикс в `logger.ts` шире Topbar — он починит любое место в коде, которое логирует `Error` как `meta`, включая предупреждения из F46.

**Файлы:** `src/components/Topbar.tsx`, `src/lib/logger.ts`.

**Верификация:** полный прогон на момент коммита — 89 файлов/742 теста зелёные, `tsc -b` чист (выделенного теста для Topbar нет — покрытие косвенное, через отсутствие регрессий в связанных сьютах).

**Статус:** ✅ исчерпывается тестами и агентской верификацией — без ручной проверки пользователя, так как находка не имеет видимого шага в интерфейсе (таймер без утечки и реальные message/stack в логах наблюдаются только в файле taskflow.log).

---

### 7.41. `pinWorkspacePointers`/`reconcileLocalPlaceholder` — один необёрнутый блок и три необёрнутых шага — падение в одном шаге гасило, прошёл ли остальные (F46, ✅ ИСПРАВЛЕНО (PR [#114](https://github.com/danny-swan/taskflow-app/pull/114), 04.08.2026), выпущено в v1.1.2 — видимо только в структуре taskflow.log, без ручной проверки)

**Симптом.** `taskflow-5.log` показывал `[sync/orchestrator] reconcilePersonalWorkspace failed` при смене аккаунта, с `meta: {}` (без деталей, до фикса F45 выше).

**Корень.** `pinWorkspacePointers` шёл одним необёрнутым блоком, а шаги 2–4 `reconcileLocalPlaceholder` (UPDATE `workspace_settings`, два UPDATE `workspace_members`, delete/rename `workspaces`) — три необёрнутых выражения подряд (шаг 1 уже имел свой try/catch из раннего фикса). Падение на шаге 2 останавливало шаги 3–4 (перекладка membership-строк и чистка placeholder-пространства) полностью, а вызывающий код видел одно непрозрачное падение без понимания, какой шаг отвалился и что из последующего выполнилось.

**Фикс.** Всё тело `pinWorkspacePointers` и каждый из шагов 2/3/4 `reconcileLocalPlaceholder` обёрнуты в свой try/catch с `logger.warn('[sync/workspace] <стадия> failed:', e)` (теперь с реальным message/stack благодаря F45). Два UPDATE шага 3 сгруппированы в один блок, так как второй зависит от успеха первого. Поведение на успехе не изменилось — те же запросы, тот же порядок, `pinWorkspacePointers` всё еще возвращает `changed`, итоговый `return true` и последующие вызовы `enqueueOutbox` не тронуты. Не устраняет саму причину возможного сбоя на шаге — только не даёт одной exception скрыть, прошли ли остальные.

**Файлы:** `src/lib/sync/workspace.ts`.

**Верификация:** 89 файлов/742 теста зелёные, `tsc -b` чист (выделенного теста нет — покрытие через сьюта sync/workspace).

**Статус:** ✅ подтверждено код-ревью + тестами (поведение на успехе не изменилось, каждый шаг теперь логирует свой сбой отдельно) — без ручного шага пользователя, так как это чисто diagnostics-фикс внутри логирования.

---

### 7.42. `sync_devices` upsert упирался в RLS при втором аккаунте на том же устройстве — `client_id` никогда не сбрасывался при смене аккаунта (F47, Bug 3, ✅ ИСПРАВЛЕНО (PR [#114](https://github.com/danny-swan/taskflow-app/pull/114), 04.08.2026), выпущено в v1.1.2 — корень доказан на реальном прод-логе, без ручного шага (проявление только в taskflow.log))

**Корень — доказан на прод-данных, не по гипотезе.** `taskflow-5.log`: `[sync/device] upsert failed` срабатывал в 20:07:39, ровно через 7 секунд после `bound local DB to user 9ef5d96b…` в 20:07:32, и повторялся 17 раз на каждом последующем цикле синха и рестарте приложения (100% отказ для аккаунта на этом устройстве, без смены аккаунта внутри этого окна) — это исключает гонку/устаревший JWT как объяснение: это детерминированный структурный конфликт, а не тайминг-проблема. `client_id` генерируется один раз на устройство (`uuidv7()` в миграции v5), кэшируется в `settings` и в модульном стейте (`src/lib/clientId.ts`) и никогда не сбрасывался при logout/смене аккаунта ни в одном реальном пути кода. Серверные RLS-политики `sync_devices` (миграция 0004, без изменений, перепроверено по `0027_workspaces_foundation.sql`) структурно корректны (`(select auth.uid()) = user_id`) — конфликт данных: когда второй аккаунт входит на устройстве, где уже есть строка `sync_devices` с тем же `client_id` от предыдущего аккаунта, UPDATE-ветвь upsert справедливо отвергается USING-условием по чужой строке.

**Фикс — только клиент, без миграции и без изменения RLS — сервер уже прав.** Новая `regenerateClientId()` (`src/lib/clientId.ts`) генерирует новый `uuidv7()`, пишет его в `settings`, обновляет кэш и возвращает новый id. `ensureDeviceRegistered()` теперь возвращает действующий `client_id` вместо `void`; при ошибке upsert по `/row-level security/i` вызывает `regenerateClientId()` и повторяет upsert ровно один раз с новым id. `syncNow()`'s локальный `clientId` теперь `let` и переприсваивается результатом `ensureDeviceRegistered()` до `pushAll()` в том же цикле — иначе самоисцеление починило бы `sync_devices`, но push в тот же цикл всё равно помечал бы строки старым, осиротевшим id. На обычном одноаккаунтном пути первый upsert успевает, `regenerateClientId()` никогда не вызывается — поведение для большинства не изменилось. `ensureDeviceRegistered()` вызывается только из pro/trial-ветки `syncNow()`, то есть free-аккаунты и бесплатные пространства этого кода вовсе не касаются.

**Файлы:** `src/lib/clientId.ts`, `src/lib/sync/index.ts`.

**Верификация:** 89 файлов/742 теста зелёные, `tsc -b` чист.

**Статус:** ✅ корень доказан на реальном прод-логе (17 повторов отказа за каждый цикл синха/рестарт), фикс подтверждён тестами — без ручного шага пользователя (симптом был виден только в файле логов, не в интерфейсе).

---

### 7.43. `pullAll()` не ревалидировал сессию после старта — выход/смена аккаунта в середине пулла давали серию `permission denied` по всем рабочим пространствам (F48, ✅ ИСПРАВЛЕНО (PR [#114](https://github.com/danny-swan/taskflow-app/pull/114), 04.08.2026), выпущено в v1.1.2 — cosmetic/log-noise фикс, без ручного шага)

**Корень — доказан на реальном прод-логе (`taskflow.log`, 22-часовое окно, 692 строки).** `syncNow()` читает сессию ровно один раз в начале (шаг 1, `supabase.auth.getSession()`), затем через несколько async-шагов вызывает `pullAll(userId)`, который последовательно делает ~2 + 9*N Postgrest-запросов (2 для фазы 1 — workspace membership, 9 на каждое из N пространств в фазе 2) и никогда повторно не валидирует сессию. Если пользователь вышел/сменил аккаунт, когда `syncNow()` уже прошёл проверку сессии, `supabase.auth.signOut()` обрывает сессию прямо посреди полёта — каждый следующий Postgrest-запрос в том же `pullAll()` уходит без валидного JWT и отвергается на уровне GRANT (Postgres 42501, `anon` не имеет грантов на `sync_*`-таблицы). В логе найдена ровно одна серия из 127 `permission denied` строк (18:31:33.908–18:31:44.864, ~11 с), раскладка по таблицам точно совпадает структурой фаза 1 + фаза 2 `pullAll`, сразу после единственного в логе одновременного закрытия обоих realtime-каналов (18:31:33.783/.788, отличается от 8 других двойных закрытий в логе, которые все — артефакт инициализации канала сразу после старта и не дают этой ошибки).

**Влияние — только шум в логах, не проблема целостности данных.** Каждый провалившийся запрос гибнет на уровне сети до того, как была бы получена хотя бы одна строка, так что `applyCloudRow*` никогда не вызывается и локальных записей нет; последующая смена аккаунта завершается корректно ~5 с спустя. Отличается от F47/Bug 3: там FK/RLS-отказ из-за конфликта `client_id`, здесь — отсутствие GRANT у роли `anon` из-за мертвой сессии, без участия `sync_devices`.

**Фикс.** `pullAll()` теперь проверяет (дешёвым локальным `supabase.auth.getSession()`, без сетевого round-trip), что сессия всё ещё принадлежит `userId` — перед стартом и повторно перед входом в фазу-2 цикл каждого пространства. Гонку до конца закрыть нельзя (сессия всё равно может умереть между отдельными вызовами внутри одного пространства), но сокращает окно отказа с N пространств до максимум 1 частичного (в наблюдавшейся ситуации ~127 предупреждений стали бы ~9–11). Если сама проверка бросает исключение, сессия считается валидной (как и раньше) — никогда не блокирует легитимный pull.

**Файлы:** `src/lib/sync/pull.ts` (+37 строк), новый `src/lib/sync/pull.f48.test.ts` (278 строк, 3 теста).

**Верификация.** 3 новых теста с моканной изменяемой auth-сессией и 2 пространствами (личное + общее, как у реального аккаунта): сессия уже изменилась/уже null до старта → 0 сетевых вызовов; сессия меняется прямо на входе в фазу-2 → ровно 9 вызовов одного рабочего пространства, второе не тронуто. Проверено через `git stash/pop`, что все 3 теста красные без фикса (18 вызовов вместо 9 — оба пространства обрабатываются, несмотря на смену сессии) и зелёные с фиксом. Полный прогон на момент коммита: 90 файлов/745 тестов (было 89/742), `tsc -b` чист.

**Статус:** ✅ принято как закрытое: корень доказан на 22-часовом реальном прод-логе (127 строк `permission denied` в одной серии), фикс подтверждён 3 новыми тестами (красные без фикса, зелёные с фиксом, проверено через git stash/pop). Прямой ручной шаг проверки невозможен (нужно воссоздать ту же гонку в прод-логах), принято по тестам + отсутствию новых ошибок в обычной работе.

---

### 7.44. Справочники статусов и тегов рендерились на двух экранах сразу, а `WorkspaceSettings.tsx` импортировал их прямо из `pages/Settings.tsx` (F49, ADR 0035, PR [#119](https://github.com/danny-swan/taskflow-app/pull/119), 05.08.2026 — ✅ ИСПРАВЛЕНО, подтверждено ручной проверкой 05.08.2026)

**Исходная жалоба:** «настройки пространства открываются двумя путями, это дублирование».

**Гипотеза пользователя не подтвердилась.** Ссылка на маршрут `/workspace-settings` в коде ровно одна — пункт «Настройки пространства» в меню переключателя пространств (`src/components/WorkspaceSwitcher.tsx:167`, открыт всем участникам по ADR 0034). Второго входа на этот маршрут нет.

**Настоящий корень — дублировался не экран, а сам справочник.** `TagsSection` (`Settings.tsx:397`) и `StatusesSection` (`Settings.tsx:464`) были объявлены в странице общих настроек и оттуда же импортировались страницей настроек пространства (`WorkspaceSettings.tsx:17`, `import { StatusesSection, TagsSection } from './Settings'`). В результате один и тот же ws-scoped справочник рендерился и как секции `statuses`/`tags` в левой навигации `/settings`, и как вкладки «Статусы»/«Теги» в `/workspace-settings`. Права нигде не расходились — оба места уже гасились `useCanManageWorkspace()` (Bug #5); расходилась модель: справочник принадлежит пространству (в общем ws его меняет владелец для всех участников), но жил в разделе, где всё остальное — настройки приложения и аккаунта. Пользователь, поменявший статусы в «Настройках», не связывал изменение с пространством.

**Решение (ADR 0035): у справочников остаётся РОВНО один экран — `/workspace-settings`.** Секции `statuses`/`tags` удалены из навигации `Settings.tsx`, из типа `Sub` и из `SETTINGS_SECTIONS` (`src/lib/settingsSections.ts`). Компоненты перенесены без функциональных изменений в новый модуль `src/components/WorkspaceReferenceSections.tsx` — исчезает импорт «страница → страница», и у ws-scoped справочника больше нет владельца в лице чужого экрана. Ролевой гейт переехал вместе с кодом.

**Файлы:** новый `src/components/WorkspaceReferenceSections.tsx` (+205), `src/pages/Settings.tsx` (−190), `src/pages/WorkspaceSettings.tsx`, `src/lib/settingsSections.ts`, новый `src/pages/settingsDedupe.test.ts`, перенацелены `src/components/editorReferenceGate.test.tsx` и `src/pages/WorkspaceSettings.test.tsx`.

**Верификация:** структурные тесты (секции удалены из `SETTINGS_SECTIONS`, `parseSettingsSection('#statuses')` = `null`, `Settings.tsx` не объявляет и не рендерит секции, `WorkspaceSettings` импортирует общий модуль, гейт роли на месте) + существующие ролевые тесты справочника, перенацеленные на новый модуль. Полный прогон: 93 файла / 763 теста, `tsc --noEmit` чист.

**Ручная проверка (нужна от пользователя):** «Настройки» → в левом списке больше нет «Статусы» и «Теги», остальные разделы на месте; переключатель пространств → «Настройки пространства» → вкладки «Статусы» и «Теги» работают как раньше (добавление, переименование, цвет, порядок, «Скрытый»/«Свёрнут», удаление); у роли editor/viewer справочник read-only с прежним хинтом.

**Статус:** ✅ ИСПРАВЛЕНО — пользователь прошёл шаги ручной проверки 05.08.2026 и подтвердил, что всё работает.

---

### 7.45. Тепловая карта «Активность · 12w» на Дашборде: строка «Пн» показывала данные другого дня недели (F50, PR [#119](https://github.com/danny-swan/taskflow-app/pull/119), 05.08.2026 — ✅ ИСПРАВЛЕНО, подтверждено ручной проверкой 05.08.2026)

**Это не визуальный дефект, а ошибка привязки данных к ячейке.** Сетка строилась в `src/pages/Dashboard.tsx` (`heatmap` useMemo) от старта `now - 12*7 + 1`, то есть «83 дня назад» — произвольный день недели. Подписи строк в компоненте `Heatmap` при этом жёстко заданы статическим массивом `['Пн','Вт','Ср','Чт','Пт','Сб','Вс']`. Совпадение подписи с содержимым получалось только когда «сегодня» — вторник; в остальные дни вся колонка была сдвинута, и сдвиг менялся каждый день.

**Доказательство на дате отчёта.** 05.08.2026 — среда → старт 14.05.2026 — **четверг** → сдвиг на 3 строки: строка «Пн» держала четверги, «Вт» — пятницы, «Ср» — субботы, «Чт» — воскресенья, «Пт» — понедельники, «Сб» — вторники, «Вс» — среды. Дата в тултипе (`title={d.date}`) при этом считалась верно — расходились именно подпись строки и содержимое ячейки, поэтому баг и выглядел как «данные не на своих местах».

**Фикс.** Построение сетки вынесено в чистую функцию `heatmapDayKeys(today)` (`src/lib/dashboard.ts`): старт = понедельник недели, отстоящей на 11 недель назад, последняя колонка = текущая неделя. Строка `j` теперь всегда соответствует `labels[j]`. Дни текущей недели после «сегодня» попадают в сетку пустыми (`count = 0`) — так неделя не разрезается посередине и подписи остаются честными. Заодно `localDayKey` переехал из страницы в `lib/dashboard.ts` (одно определение на проект).

**Файлы:** `src/lib/dashboard.ts` (+`heatmapDayKeys`, +`localDayKey`, +`HEATMAP_WEEKS`), `src/pages/Dashboard.tsx`, новый `src/lib/dashboardHeatmap.test.ts` (6 тестов).

**Верификация:** тест прогоняет инвариант «строка `j` = день недели `j`» для всех 7 вариантов дня «сегодня», проверяет 12×7 = 84 уникальных подряд идущих дня без дыр, переход через границу года, наличие «сегодня» в последней колонке и отдельно фиксирует регрессию — старая формула «−83 дня» на 05.08.2026 даёт старт-четверг, новая даёт понедельник.

**Ручная проверка (нужна от пользователя):** Дашборд → «Активность · 12w» → навести на несколько ячеек в разных строках и сверить дату из тултипа с подписью строки слева (ячейка в строке «Пн» должна давать понедельник, в «Вс» — воскресенье); проверить, что последняя (правая) колонка — текущая неделя.

**Статус:** ✅ ИСПРАВЛЕНО — пользователь прошёл шаги ручной проверки 05.08.2026 и подтвердил, что всё работает.

---

### 7.46. В «Истории» у самого себя рисовался аватар-заглушка, хотя во вкладке «Участники» аватар был верный (F51, PR [#119](https://github.com/danny-swan/taskflow-app/pull/119), 05.08.2026 — ✅ ИСПРАВЛЕНО, подтверждено ручной проверкой 05.08.2026)

**Корень — в ветке «это я» задавалось только имя.** В `ActivityAuthorRow` (`src/components/ActivityEntry.tsx`, общий рендер для `TaskActivityLog` и `WorkspaceHistoryTab`) переменные инициализируются как `variant = 1`, `color = null`, и ветка `if (boundUserId && record.userId === boundUserId)` присваивала только `name = tr(lang, 'ws_activity_you')`. Ветки `presence` и `cached` рядом задают и `variant`, и `color` — то есть чужие аватары рисовались правильно, а свой всегда оставался дефолтным.

**Усиливающий фактор.** Фолбэка через presence для себя не существует по замыслу: `usePresenceStore.byId` сознательно не содержит собственного пользователя (`src/lib/presence.ts:90`).

**Почему «Участники» показывали верно.** `MembersTab.tsx:126` читает `memberProfiles[user_id]` для всех подряд, включая себя, — RPC `get_workspace_member_profiles` (ADR 0031) возвращает и вызывающего. Проверено на проде: `TF-7EDA2R` имеет `avatar_variant = 5`, `avatar_color = #fc1d1d`, то есть данные для правильного аватара были доступны клиенту всё это время.

**Фикс.** В ветке «это я» подпись остаётся «вы», но `variant`/`color` берутся из кэша публичных профилей (F40, ADR 0031) — того же источника, что и для остальных участников. Чтобы кэш был заполнен и там, где RPC участников не вызывается (личное пространство, первый запуск), `ProfileBlock` кладёт собственный публичный минимум в тот же кэш новым хелпером `upsertMemberProfileCacheEntry()`. `email` в кэш не попадает. Пустой кэш = прежнее поведение (вариант 1), без падений.

**Файлы:** `src/components/ActivityEntry.tsx`, `src/lib/memberProfiles.ts` (+`upsertMemberProfileCacheEntry`), `src/components/ProfileBlock.tsx`, новый `src/components/activitySelfAvatar.test.tsx` (4 теста).

**Верификация:** тесты проверяют, что подпись остаётся «вы», что цвет из профиля реально доезжает до inline-стиля аватара, что своя и чужая строки дают разные аватары, и что без кэша рендер не падает. Миграций нет, RLS не менялась.

**Ручная проверка (нужна от пользователя):** открыть задачу → «История изменений» и «Настройки пространства» → «История»; свои записи должны идти с вашим аватаром (форма и цвет как в «Настройки → Аккаунт → Профиль») и подписью «вы», чужие — как раньше. Если аватар всё ещё дефолтный, сначала зайдите один раз в «Настройки → Аккаунт» (там свой профиль попадает в кэш) и вернитесь в историю.

**Статус:** ✅ ИСПРАВЛЕНО — пользователь прошёл шаги ручной проверки 05.08.2026 и подтвердил, что всё работает.

---

### 7.47. Кнопка «Оплатить» в тарифе «Ежегодно» — белый текст на светлой карточке: страница `/checkout` свёрстана на несуществующих цветовых токенах (F52, PR [#119](https://github.com/danny-swan/taskflow-app/pull/119), 05.08.2026 — ✅ ИСПРАВЛЕНО, подтверждено ручной проверкой 05.08.2026)

**Это не «неудачный оттенок», а несуществующие классы.** В `tailwind.config.ts` объявлены только цвета `bg`, `surface`, `surface-alt`, `border`, `border-soft`, `text`, `muted`, `faint`, `accent`, `accent-hover`, `accent-soft`. Токенов `primary` и `success` нет ни в конфиге, ни в `src/styles/globals.css` (там есть `--status-done`, `--status-important`, `--status-overdue`, `--status-paused`, `--status-progress`, `--status-take*`, `--status-today`, `--status-warn`). При этом `src/pages/Checkout.tsx` использовал `bg-primary` ×13, `text-primary` ×12, `border-primary` ×3, `ring-primary` ×1, `bg-success` ×1, `border-success` ×1, `text-success` ×4 — Tailwind для неизвестного цвета молча не генерирует правило, поэтому все они были no-op.

**Почему пострадал ровно один тариф.** Класс кнопки: `tier.highlight ? 'bg-primary text-white hover:bg-primary/90' : 'bg-surface-alt border border-border hover:border-primary/50'`. `highlight: true` стоит только у `annual` — у него фон не применялся вовсе, а валидный `text-white` оставался, отсюда белый текст на светлой карточке. «Ежемесячно» и «Навсегда» брали валидный `bg-surface-alt` и выглядели нормально. Тем же дефектом были сломаны бейдж «Популярное», подсветка предвыбранной карточки и зелёный баннер тарифа Lifetime.

**Фикс.** Страница переведена на существующие токены: `primary` → `accent` / `accent-hover` / `accent-soft`, `success` → arbitrary-значение `[var(--status-done)]`. Проверено компиляцией Tailwind по исходнику `Checkout.tsx`: до фикса правила для `bg-primary`/`text-primary` не генерировались вообще, после — все использованные классы дают реальные CSS-правила.

**Файлы:** `src/pages/Checkout.tsx` (+шапка с правилом на будущее), тесты в `src/pages/settingsDedupe.test.ts`.

**Верификация:** тесты статически проверяют, что в `Checkout.tsx` не осталось классов `(bg|text|border|ring|from|to|via)-(primary|success)`, что в `tailwind.config.ts` этих цветов по-прежнему нет, и что у выделенного тарифа фон и `text-white` заданы вместе (`bg-accent text-white hover:bg-accent-hover`).

**Ручная проверка (нужна от пользователя):** «Настройки → Подписка → Оформить» (или `/checkout`) → у тарифа «Ежегодно» кнопка «Оплатить» с акцентным фоном и читаемым белым текстом; бейдж «Популярное», рамка предвыбранной карточки и баннер тарифа «Навсегда» видны; проверить во всех темах, включая тёмную.

**Статус:** ✅ ИСПРАВЛЕНО — пользователь прошёл шаги ручной проверки 05.08.2026 и подтвердил, что всё работает.

---

### 7.48. `src/pages/AdminPage.tsx` содержит тот же дефект несуществующих токенов, что и Checkout (F53, 🟡 не чинилось — сознательно вне объёма PR #119)

Обнаружено при разборе F52. Админская страница использует те же несуществующие классы `*-primary` / `*-success`, то есть часть её акцентов и статусных цветов не применяется. По решению пользователя объём PR #119 ограничен экраном `/checkout`, поэтому находка зафиксирована отдельным пунктом и не правилась.

**Влияние:** только внешний вид внутренней админ-страницы; логика, права и данные не затронуты. **Действие:** привести `AdminPage.tsx` к токенам темы тем же способом, что и `Checkout.tsx` (см. §7.47), отдельным PR.

---

### 7.49. Модификатор прозрачности (`bg-accent/40`) к var-цветам в Tailwind 3.4 не применяется — правило не генерируется (F54, 🟡 риск, не чинилось)

Обнаружено при подборе замены токенов для F52. Все цвета темы объявлены как `var(--…)`, и для таких значений Tailwind 3.4.19 **не генерирует правило с opacity-модификатором вовсе** — проверено прогоном компилятора: `bg-accent/40` и `bg-[var(--status-done)]/10` не дают ни одного CSS-правила, тогда как `bg-accent`, `bg-accent-soft`, `text-[var(--status-done)]` работают.

**Следствие:** в проекте есть места, где прозрачность молча теряется, например `bg-accent/40` в `InviteMemberModal.tsx:139`, `MembersTab.tsx:104`, `CreateWorkspaceModal.tsx:134` (кнопка в disabled-состоянии выглядит как обычная), `ring-accent/40` в `AvatarEditorModal.tsx:156`, `hover:border-accent/50` и `hover:bg-accent/5` в `AccountSwitchGate.tsx`. Это не ломает функциональность, но означает, что часть задуманных «приглушённых» состояний не отображается.

**Возможные направления:** либо задавать цвета в конфиге через `rgb(var(--…) / <alpha-value>)` с переводом переменных в channel-формат, либо явно объявить нужные полупрозрачные токены (`accent-soft` уже так и сделан), либо оставить как есть и не использовать `/NN` на цветах темы. Решение принять отдельно — затрагивает всю палитру.

---

### 7.50. Компонент `Heatmap` красит ячейки через CSS `color-mix`, от которого ADR 0032 сознательно отказался (F55, 🟡 риск, не чинилось)

Обнаружено при разборе F50. `src/pages/Dashboard.tsx`, компонент `Heatmap`: фон ячейки задаётся как `color-mix(in srgb, var(--accent) N%, transparent)`. ADR 0032 (F41) специально ушёл от `color-mix` в аватарах на ручной `rgba()` — из-за системного WebView в Tauri, где на поддержку `color-mix` полагаться нельзя. Если WebView его не поддержит, ячейки с активностью получат невалидный фон и станут визуально неотличимы от пустых.

**Не чинилось в PR #119 сознательно:** это отдельная гипотеза о рендеринге, а не доказанный на данных пользователя баг, и трогать окраску одновременно с исправлением привязки данных (F50) значило бы смешать в одном изменении проверяемое и непроверяемое. **Действие:** подтвердить или опровергнуть на реальной сборке под Tauri, при подтверждении — перевести на `avatarTint`-подобный ручной `rgba()`, как в ADR 0032.

---

### 7.51. Приглашение в сайдбаре не показывало ни названия пространства, ни того, кто пригласил (F56, ADR 0036, миграция 0044, PR [#120](https://github.com/danny-swan/taskflow-app/pull/120), 05.08.2026 — 🟡 ждёт ручной проверки пользователем)

**Это не недоделка UI, а RLS.** Карточка входящего приглашения (`src/components/MyInvitesSection.tsx`, левое меню под переключателем пространств) выводила нейтральное «Приглашение в общее пространство» + роль и срок. Причина: приглашённый **ещё не участник** пространства, поэтому отображаемые поля ему закрыты. Проверено на проде под ролью приглашённого (`set local role authenticated` + `request.jwt.claim.sub`, всё в откатываемой транзакции):

| Запрос от лица приглашённого | Было |
| --- | --- |
| `select name from sync_workspaces where id = <ws>` | 0 строк |
| `select public_user_id, nickname from profiles where id = <inviter>` | 0 строк |
| `select * from sync_workspace_invites where target_user_id = auth.uid()` | 1 строка (только идентификаторы) |

Прежнее решение Wave B (PR-b-04) это зафиксировало сознательно — «approach 5.b, без backend-правок» (`docs/architecture/wave-b-plan.md` §4.4-факт). Обойти RLS на клиенте нельзя в принципе, поэтому исправление обязано быть серверным.

**Решение (ADR 0036).** Миграция **0044** — `SECURITY DEFINER STABLE` RPC `public.get_my_pending_invites()` **без параметров**, гейт `i.target_user_id = auth.uid()`: отдаёт вызывающему его pending-приглашения вместе с `workspace_name` и публичным минимумом профиля пригласившего (`public_user_id`, `nickname`, `avatar_variant`, `avatar_color`, `bio`). Email и приватные поля не возвращаются. Гранты по ADR 0002 (`revoke … from anon, public`; `grant execute … to authenticated`). Фильтр строк совпадает с прежним клиентским SELECT, так что список приглашений не изменился — изменился состав колонок.

**Проверка на проде (в откатываемой транзакции, изменения откачены).** Инвайт от `TF-7EDA2R` (Leaf team) к `TF-Z8TSB7`: до фикса имя ws и профиль пригласившего давали 0 строк, RPC вернула `Leaf team | TF-7EDA2R | Daniil Lebedev | v5 | #fc1d1d | editor`. Отдельно проверено: чужое приглашение в выдачу не попадает, `accepted` не попадает, у мягко удалённого пространства имя всё равно отдаётся.

**Клиент.** `listMyPendingInvites()` переведён на RPC; прежний прямой SELECT оставлен **fallback'ом** — если миграция ещё не применена (по ADR 0027 это ручное действие) или RPC недоступна, список приглашений работает как раньше, просто без обогащения. В карточке: заголовок = название пространства, ниже строка «От: <аватар> <ник или TF-id>», клик по ней открывает ту же `MemberInfoModal`, что и список участников (ADR 0031) — новых экранов не заводили.

**Файлы:** новые `supabase/migrations/0044_my_pending_invites_rpc.sql` и `supabase/tests/23_my_pending_invites_test.sql`; `src/lib/invites.ts` (+`MyPendingInvite`, +`normalizeMyPendingInviteRow`, RPC + legacy-fallback), `src/store/useInvitesStore.ts` (тип `myPending`), `src/components/MyInvitesSection.tsx`, тесты `src/lib/invites.test.ts` и `src/components/MyInvitesSection.test.tsx`.

**Верификация:** pgTAP `23_my_pending_invites_test.sql` (14 проверок: гранты authenticated/anon, SECURITY DEFINER, STABLE, отсутствие email в сигнатуре, ноль параметров, все три «корневых» замера RLS, выдача с именем ws и профилем, отсечение чужого и не-pending, мягко удалённое ws); vitest — 4 теста контракта RPC/fallback + 5 тестов карточки. Полный прогон и db-tests — зелёные.

**Ручная проверка (нужна от пользователя):** со второго аккаунта пригласить свой основной по TF-id в общее пространство → в левом меню карточка должна показать **название этого пространства** и строку «От: …» с аватаром и ником (или TF-id, если ник не задан); клик по строке открывает карточку профиля с TF-id и «о себе»; «Принять» и «Отклонить» работают как раньше.

**Статус:** 🟡 изменение внесено, ждёт ручной проверки пользователем.
...
| **0042** | profile_avatar_color | **F41 (⏳ применение на прод — вручную через `apply_migration` с confirm_action, ветка `feat/member-profiles-account-redesign`, ADR 0032):** `alter table public.profiles add column if not exists avatar_color text` + CHECK `profiles_avatar_color_format` (`~* '^#[0-9a-f]{6}$'` или NULL) + comment. Цвет аватара становится явным свойством профиля вместо производной темы приложения (`NULL` = не задан → прежнее поведение). Требуется применить ДО выпуска сборки: клиент запрашивает колонку в `PROFILE_COLUMNS`. pgTAP `21_member_profiles_test.sql`. См. §7.36 (F41) |
| **0043** | workspace_member_profiles_rpc | **F40 (⏳ применение на прод — вручную через `apply_migration` с confirm_action, ветка `feat/member-profiles-account-redesign`, ADR 0031):** новая SECURITY DEFINER STABLE RPC `public.get_workspace_member_profiles(p_workspace_id text)` (`set search_path = public, pg_catalog`) — join `sync_workspace_members` × `profiles`; гейты: `auth.uid() is not null`, `deleted_at is null`, `has_workspace_role(p_workspace_id, auth.uid(), 'viewer')`; returns `(user_id, public_user_id, nickname, avatar_variant, avatar_color, bio)` — **без `email`**; `REVOKE ALL FROM public, anon` + `GRANT EXECUTE TO authenticated` (паттерн ADR 0002). Корень: own-row RLS на `profiles` не даёт клиенту читать чужие ники/TF-id, поэтому в списке участников показывался кусок uuid. RLS `profiles` и логика членства/инвайтов НЕ менялись. pgTAP `21_member_profiles_test.sql` (`plan(13)`). Прод-проба под ROLLBACK 03.08.2026 пройдена (участник видит 2 профиля с TF-id/ником/цветом, не-участник — 0 строк). См. §7.35 (F40) |
