...
| N8 🟡 | Webhook доверяет `X-Forwarded-For` в IP-whitelist (подделываемый заголовок) + вебхук диспетчеризовал по `event` из тела, не сверяя со статусом реального платежа | `payment-webhook/index.ts` | MEDIUM | **🟡 ИСПРАВЛЕНО в `develop` (Wave 3, PR #65), деплой не подтверждён.** `_shared/yookassa-verify.ts::assessVerifiedPayment` строго сверяет `event` со `status` из dual-verify (`GET /v3/payments/{id}`) — подделанное `payment.succeeded` по pending/canceled больше не активирует подписку. ⏳ ждёт деплоя `payment-webhook` v24. Отдельно: убедиться, что `YOOKASSA_SKIP_IP_CHECK` выключен в проде |
...
| N9 🟡 | Нет сверки суммы платежа с прайсом в webhook | `payment-webhook` | LOW | **🟡 ИСПРАВЛЕНО в `develop` (Wave 3, PR #65), деплой не подтверждён.** `_shared/pricing.ts::verifyPaymentAmount` сверяет `amount.value`+currency против серверного `TIER_PRICING` (единый источник истины). ⏳ ждёт деплоя `payment-webhook` v24 |
| N10 🟡 | Идемпотентность зависит от `attempt_no` — при timeout возможен повтор | `renew-subscription` | LOW | **🟡 ИСПРАВЛЕНО в `develop` (Wave 3, PR #65, см. ADR 0003), деплой не подтверждён.** `_shared/renewal-idempotency.ts::selectActiveRenewalPayment` — сверка `GET /v3/payments` до создания платежа: активный (pending/waiting_for_capture/succeeded) → второй не создаём. ⏳ ждёт деплоя `renew-subscription` v10 |
...
3. **Wave 3 — ✅ ЗАВЕРШЁН (10.07.2026, ветка `wave-3-hardening` → PR #65, merge `49ec66d`):** N6 (default privileges) + N9 (сверка суммы) + N8 (dual-verify assessment) + N10 (идемпотентность) + F3 (Settings.tsx card_brand→card_type) + CI-пробел (Deno edge-job). N6 — миграция `0021`, применена на прод. N9/N8/N10 — код edge-функций в `develop`; ⚠️ деплой `payment-webhook` v24 / `renew-subscription` v10 в сессии не выполнен (нет доступа к supabase-коннектору) — см. «Wave 3 — итог» ниже. F3 — фронт, требует релиза десктопа.
...
⚠️ **Деплой edge-функций (payment-webhook v24, renew-subscription v10) в этой сессии НЕ выполнен** — у автоматизации не было доступа к supabase-коннектору/`SUPABASE_ACCESS_TOKEN`. Код в `develop`; требуется отдельный деплой обеих функций с `verify_jwt=false` (см. блок «Требуется отдельный деплой» ниже). Пока деплой не подтверждён через `list_edge_functions`, N9/N8/N10 остаются «в develop, НЕ на проде».
...
| **N9** (сверка суммы платежа с прайсом) | 🟡 В `develop`, деплой не подтверждён | `_shared/pricing.ts` (`TIER_PRICING`/`verifyPaymentAmount`/`isRecurringTier`), импортится в `payment-webhook` + `renew-subscription` | ⏳ ждёт деплоя `payment-webhook` v24 + `renew-subscription` v10 |
| **N8** (dual-verify assessment gap) | 🟡 В `develop`, деплой не подтверждён | `_shared/yookassa-verify.ts::assessVerifiedPayment` (сверка `event` уведомления со `status` из dual-verify) | ⏳ ждёт деплоя `payment-webhook` v24 |
| **N10** (renewal idempotency-guard) | 🟡 В `develop`, деплой не подтверждён | `_shared/renewal-idempotency.ts::selectActiveRenewalPayment` (сверка GET /v3/payments до создания платежа); см. ADR 0003 | ⏳ ждёт деплоя `renew-subscription` v10 |
...
**Требуется отдельный деплой (не выполнено в этой сессии):**
- `payment-webhook` (v23 → **v24**), `verify_jwt=false`, файлы: `index.ts` + `../_shared/pricing.ts` + `../_shared/yookassa-verify.ts`.
- `renew-subscription` (v9 → **v10**), `verify_jwt=false`, файлы: `index.ts` + `../_shared/pricing.ts` + `../_shared/renewal-idempotency.ts`.
- После деплоя: подтвердить версии через `list_edge_functions` (v24/v10) и обновить столбец «Применение на прод» выше на ✅.
...
## Приложение А. Полная таблица миграций Supabase (0001-0021)
