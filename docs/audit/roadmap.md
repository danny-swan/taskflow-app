...
> Серия из трёх PR-ов по багам/фичам, смёржены в `main` 12.07.2026. Тег v1.0.2 **пока не выпущен** — требуется version bump `package.json` и релиз десктопа отдельным заходом (см. раздел 7.4). Серверная часть (миграция `0025`) — НЕ применена на прод (клиент-код уже в `main`, но пользователей на v1.0.2 ещё нет, поэтому пуша в несуществующую таблицу не происходит).
...
### 7.4. Релиз v1.0.2 (не выпущен)

**Накоплено в `main` на 12.07.2026:**
- B1 — фикс «Вернуть в работу» (PR #70).
- B2 — фикс race первого клика по «Администрирование» (PR #71).
- F6 (фронт) — столбец «Холд» + клиент-миграция v10 (PR #72).

**Что требуется для выпуска v1.0.2:**
1. Bump `package.json` version → `1.0.2`.
2. Сборка десктоп-артефактов (Tauri NSIS+MSI+portable+dmg) через тег `v1.0.2` → CI `build.yml`.
2a. Перед выпуском v1.0.2 — **применить миграцию `0025_task_hold_periods.sql`** через Supabase-коннектор (иначе клиент v1.0.2 начнёт push-ить в несуществующую таблицу). Порядок: migration → tag/release.
3. Publish [Releases](https://github.com/danny-swan/taskflow-app/releases) с билингвальными release notes.

**Содержание v1.0.2 (черновик):** «UX-фиксы после v1.0.1: возврат задачи в работу (B1), вход в админку с первого клика (B2), точный счёт дней в столбце «Холд» в статистике задач (F6, новая таблица `task_hold_periods`)».
...
## Приложение А. Полная таблица миграций Supabase (файлы 0001–0025, применено на проде 21 миграция; 0025 НЕ применена — ожидает выпуска v1.0.2)
...
| **0025** | task_hold_periods | **Post-v1.0.1 (🟡 НЕ ПРИМЕНЕНА НА ПРОД на 12.07.2026):** F6 — таблица `task_hold_periods` (task_id/started_at/ended_at/user_id + version/client_id/deleted_at) + RLS own-row + GRANT authenticated (SELECT/INSERT/UPDATE) + BEFORE-UPDATE trigger `updated_at` + realtime + индексы (`task_id`, `user_id`, частичный `WHERE ended_at IS NULL`) + идемпотентный бэкфилл для тасков в текущем статусе «Приостановлено» (INSERT одной открытой строки с `started_at = tasks.updated_at`). Клиент (PR #72) — единственный автор интервалов (серверного триггера нет). Покрыто pgTAP `07_task_hold_periods_test.sql`. **Применение на прод — перед выпуском v1.0.2** (см. раздел 7.4) |
