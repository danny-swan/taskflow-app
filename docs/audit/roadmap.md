...
| # | Находка | Где | Критичность | Что делать |
|---|---|---|---|---|
| N4 | View `admin_users_summary` без `security_invoker`/REVOKE — потенциальная утечка email всех юзеров | `migrations/0001_init.sql:106-120` | MEDIUM | `security_invoker=on` + REVOKE от anon/authenticated + pgTAP-тест |
| N5 | View `sync_status_summary` — аналогично, раскрывает кросс-пользовательские метрики | `migrations/0002_sync_schema.sql:344-362` | MEDIUM | Та же рекомендация, либо `where u.id = auth.uid()` |
...
| **N15** 🔴 | **(новое, 10.07)** RPC `get_users_emails(user_ids uuid[])` — `SECURITY DEFINER`, вызываем любым `authenticated`-юзером через `/rest/v1/rpc/get_users_emails`, без видимого ограничения на чужие user_id | Postgres function `public.get_users_emails` (подтверждено Supabase Advisors) | **MEDIUM-HIGH** | Проверить все вызовы в коде; если только для admin-контекста — `REVOKE EXECUTE FROM authenticated`, оставить только service_role/явную admin-проверку внутри функции |
...
2. **Wave 2:** N4 + N5 (views, утечка PII) + N12 (RLS без WITH CHECK) + **N15** (RPC-утечка email — приоритет как у N4/N5, возможно выше)
...
3. 🔍 **Проверяется:** после фикса F4 теоретически возможны дублирующиеся письма `renewal_failed` (один и тот же провал теперь может обрабатываться и webhook-путём, и cron-путём `renew-subscription`). По запросу пользователя проводится анализ кода обоих путей; если дубль подтвердится — добавить дедупликацию по `payment_id`.
