...
- 🟡 Две view в Supabase потенциально раскрывают email/метрики всех пользователей любому залогиненному юзеру (**N4, N5**).
...
2. **Wave 2:** **F5** (дубль `renewal_failed`, edge `renew-subscription`) + N4 + N5 (views, утечка PII) + N12 (RLS без WITH CHECK) + **N15** (RPC-утечка email — приоритет как у N4/N5, возможно выше). N4/N5/N12/N15 — миграция Supabase; F5 — деплой edge-функции (два разных вида применения на прод)
