...
**Открытое после hotfix (см. `taskflow_plan_next_session.md`):** отдельный P0-баг — **принятие приглашения не показывает приглашённому чужое shared-пространство** (гипотеза: pull фильтрует по `user_id`, чужие ws-строки не тянутся) — чинится следующим заходом. **Ретроспектива отладки** (почему корень 403 нашли поздно и как надо было: сначала захватить реальный запрос/ответ, воспроизводить тем же клиентским путём с RETURNING, сразу поднять локальный pgTAP и писать RED-тест) — вынесена в `taskflow_plan_next_session.md` и ограничена областью workspaces/RLS/sync.
...
> Файлы `0001`–`0026` (платежи/профиль) применены на проде на 12.07.2026. Файлы `0027`–`0037` — эпик «Пространства» (Wave A/B/C + hotfix), живут в `feat/workspaces`; статус применения на прод указан в строке каждой миграции. Историю строк не переписываем — новые миграции дописываются в конец таблицы (см. `roadmap-guidelines.md`).
