...
> Базовая кастомизация профиля: публичный ID + профильные поля. **PR [#73](https://github.com/danny-swan/taskflow-app/pull/73)** — ОТКРЫТ (не смёржен на 12.07.2026). Серверная миграция `0026_profile_customization.sql` — ✅ ПРИМЕНЕНА НА ПРОД 12.07.2026.
...
**Генерация public ID:** функция `gen_public_user_id()` (алфавит `A-Z` + `2-9` без `I L O 0 1`) + `assign_public_user_id()` (retry ≤10 при коллизии). Новым юзерам — в триггере `handle_new_user`. Уникальность — UNIQUE-constraint `profiles_public_user_id_key`.
