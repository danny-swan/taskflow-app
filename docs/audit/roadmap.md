...
### 7.32. Задача, созданная за секунды до выхода из free-аккаунта, могла пропасть: слот не писался при logout + записи в нативную SQLite шли без порядка и без точки ожидания (F37, ⚠️ реализовано, тесты зелёные, ждёт ручную проверку, 03.08.2026)
...
**Статус:** ⚠️ реализовано, ждёт ручной проверки пользователя: создать задачу и в течение 2–3 с выйти из аккаунта → войти в другой → вернуться (задача на месте); повторить с перезапуском приложения между выходом и возвратом.
