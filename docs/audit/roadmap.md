...
### 7.5. Пространства (Workspaces) — Wave A (в разработке)
...
- **PR-5 (тарифные лимиты) — реализован**, ждёт push/ревью: серверный триггер `enforce_workspace_limit` + `get_workspace_limit` (`0029`), Free = 2 / Pro = 7, UX-гейт в модалке + fallback на sync-ошибку. pgTAP `11_workspace_limits_test.sql`.
- **PR-6 (hardening/доки) — реализован**, ждёт push/ревью: regression-pgTAP `12_workspaces_regression_test.sql` (`plan(45)`: 18 RLS-изоляция двух юзеров / 12 удаление ws / 15 integrity) в CI; локально 11 файлов / 310 тестов PASS на PG15-совместимых ассершенах. Продуктовых изменений нет.
...
- **Shared-пространства (Wave B)** — ещё НЕ открыты: `kind='shared'` заблокирован check-constraint'ом + триггером; лимитная логика уже форвард-совместима.
