...
- ✅ Две view в Supabase потенциально раскрывают email/метрики всех пользователей любому залогиненному юзеру (**N4, N5**) — закрыто в Wave 2 (миграция `0020`, `security_invoker=on` + REVOKE); ещё не применено на прод.
...
2. **Wave 2 — ✅ ЗАВЕРШЁН в коде (10.07.2026, ветка `wave2-fixes`):** **F5** (дубль `renewal_failed`, edge `renew-subscription`) + N4 + N5 (views, утечка PII) + N12 (RLS без WITH CHECK) + **N15** (RPC-утечка email — приоритет как у N4/N5, возможно выше). N4/N5/N12/N15 — миграция Supabase `0020`; F5 — деплой edge-функции (два разных вида применения на прод). Ещё НЕ на проде: миграция не применена, edge-функция не задеплоена — см. "Wave 2 — итог" ниже
...
Код: ветка `wave2-fixes` (атомарные коммиты). **Ещё НЕ на проде** — требуется применение миграции + деплой edge-функции.

| Находка | Статус | Где живёт фикс | Применение на прод |
|---|---|---|---|
| **F5** (дубль письма/счётчика/лога) | ✅ В коде | `supabase/functions/renew-subscription/index.ts` (синхр. `canceled`-ветка) + unit-тест `test.ts` | Деплой edge-функции `renew-subscription` |
| **N4** (`admin_users_summary` security_invoker) | ✅ В коде | миграция `0020_wave2_security_hardening.sql` | Применить миграцию |
| **N5** (`sync_status_summary` security_invoker) | ✅ В коде | миграция `0020` | Применить миграцию |
| **N12** (`profiles_update_own` WITH CHECK) | ✅ В коде | миграция `0020` | Применить миграцию |
| **N15** (RPC `get_users_emails` admin-гейт) | ✅ В коде | миграция `0020` (см. ADR 0002) | Применить миграцию |
...
1. Миграция `0020` идемпотентна — но применять на прод только после ревью; порядок: сначала миграция (N4/N5/N12/N15), затем деплой `renew-subscription` (F5) — независимы, но оба нужны.
...
3. Docs-файлы (`docs/audit/roadmap.md`, `docs/adr/*`, `docs/architecture/erd.md`) исходно жили только на ветке `docs/setup-adr-audit`, а не на `develop`; в `wave2-fixes` они принесены из неё, чтобы обновление статуса было консистентным. При мерже согласовать с веткой документации, чтобы не задублировать.
...
## Приложение А. Полная таблица миграций Supabase (0001-0019)
